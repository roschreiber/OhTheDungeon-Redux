...
public class Pool extends FeatureAdder {
	...
	public void buildFeature(Dungeon dungeon, Room room) {
		...
		case TR1: {
			...
			which[(rotation + 1) % 4].drawLiquid(dungeon, room, oppX, oppZ, dimX, dimZ, false, false);
		}
			break;
		case TR2: {
			oppX = room.realX + ((centerZ - room.realZ) / (room.endZ - room.beginZ)) * (room.endX - room.beginX);
			...
			which[(rotation + 1) % 4].drawLiquid(dungeon, room, oppX, oppZ, dimX, dimZ, false, true);
		}
			break;
		case X: {
			which[rotation].drawLiquid(dungeon, room, oppX, centerZ, dimX, dimZ, true, false);
		}
			break;
		case Z: {
			which[rotation].drawLiquid(dungeon, room, centerX, oppZ, dimX, dimZ, false, true);
		}
			break;
		case XZ: {
			which[rotation].drawLiquid(dungeon, room, oppX, centerZ, dimX, dimZ, true, false);
			which[rotation].drawLiquid(dungeon, room, centerX, oppZ, dimX, dimZ, false, true);
			which[rotation].drawLiquid(dungeon, room, oppX, oppZ, dimX, dimZ, true, true);
		}
			break;
		case R: {
			which[(rotation + 2) % 4].drawLiquid(dungeon, room, oppX, oppZ, dimX, dimZ, false, false);
		}
			break;
		case SW: {
			...
			which[(rotation + 1) % 4].drawLiquid(dungeon, room, swX2, swZ1, dimX, dimZ, false, false);
			which[(rotation + 3) % 4].drawLiquid(dungeon, room, swX1, swZ2, dimX, dimZ, false, false);
			which[(rotation + 2) % 4].drawLiquid(dungeon, room, oppX, oppZ, dimX, dimZ, false, false);
		}
		}
	}
