 ...
 * jar by URI and write them to external files. More to the point, this if for
...
public class Externalizer {
	...
	private static final String[] themes = new String[] { "common.cfg", "continentalShelf.cfg", "dank.cfg",
			"desert.cfg", "frozen.cfg", "jungle.cfg", "mesa.cfg", "nether.cfg", "oceanic.cfg", "template.cfg",
			"urban.cfg", "villagelike.cfg", "volcanic.cfg" };
	...
	/**
	 * Exports a theme file by reading it from the jar and writing it to the hard
	 * drive, but only if a file with that path does not already exist on the drive.
	 * 
	 * @param name
	 */
	...
	public void copyOut(String name, String outloc) {
		try {
			...
			if (outFile.exists())
				return;
			instream = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream(baseLocation + name)));
	...
	/**
	 * Exports a theme file by reading it from the jar and writing it to the hard
	 * drive, over-writing any file with the same path.
	 * 
	 * @param name
	 */
