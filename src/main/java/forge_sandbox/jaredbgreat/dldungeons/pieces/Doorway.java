...
/**
 * This class represents a doorway for use with classes in the astar package,
 * notable DoorChecker and AStar.
 * 
 * Note that this represents the door in relation to a specific room; the same
 * room will have a separate listing for each room it connects within that rooms
 * list of doors.
 * 
...
public class Doorway extends Tile implements Comparable<Doorway> {
	...
	/**
	 * How desirable this door is as the primary connection between rooms; this is
	 * effected by its position in relation to pool and its previous uses in
	 * connecting rooms.
	 */
	public int priority;
	/**
	...
	/**
	 * This will find the number of sides bordering a "liguid" pool and add it to
	 * the priority value; since the Java priority queue sorts to lowest value first
	 * increasing this effectively decreases the priority so that pools with no or
	 * fewer pools beside them will be chosen first as doors for AStar to connect.
	 * 
	 * @param dungeon
	 * @param start
	 */
	public void prioritize(Dungeon dungeon, int start) {
		if (xOriented) {
			if (dungeon.map.hasLiquid[x + 1][z])
				priority++;
			if (dungeon.map.hasLiquid[x - 1][z])
				priority++;
			...
			if (dungeon.map.hasLiquid[x][z + 1])
				priority++;
			if (dungeon.map.hasLiquid[x][z - 1])
				priority++;
