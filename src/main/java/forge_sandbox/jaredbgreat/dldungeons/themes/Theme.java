...
/**
 * Themes represent dungeon styles, including blocks and mobs to use; and
 * variables influencing architectural style elements such as the amount of
 * symmetry, complexity, and height variability; and the frequency of various
 * features such as platforms, sub-rooms, and pools of liquid.
 * 
 * Each theme is tagged to appear in certain biome types and at certain
 * altitudes.
 * 
 * As with Room, this class is not truly encapsulated. While this could be
 * fixed, so much of the code has since been written based on the unencapsulated
 * version that at this time there is no plan to improve this.
 * 
...
public class Theme {
	...
	public int[] getUpdatedRange(AsyncWorldEditor aworld) {
		return aworld.getUpdatedRange(minY, maxY);
	}
	...
	public Element degeneracy; // Chance of walls / ceilings not spawning over air blocks (idea taken from
								// Greymerk)
	...
	public Element entrances; // Ways in and out / number of entrance/exit nodes (others are destination /
								// treasure nodes)
	...
	public void addMob(String mob, int level) {
		switch (level) {
		case 0:
			if (!commonMobs.contains(mob))
				commonMobs.add(mob);
			return;
		case 1:
			if (!hardMobs.contains(mob))
				hardMobs.add(mob);
			return;
		case 2:
			if (!bruteMobs.contains(mob))
				bruteMobs.add(mob);
			return;
		case 3:
			if (!eliteMobs.contains(mob))
				eliteMobs.add(mob);
			return;
		default:
	...
	/**
	 * Remove a mob from the list of mobs with the given difficulty level. This
	 * primarily for use with API to allow other mods to remove mobs, presumably
	 * when replacing then with an alternative.
	 * 
	 * @param mob
	 * @param level
	 */
	public void removeMob(String mob, int level) {
		switch (level) {
		case 0:
			while (commonMobs.contains(mob))
				commonMobs.remove(mob);
			return;
		case 1:
			while (hardMobs.contains(mob))
				hardMobs.remove(mob);
			return;
		case 2:
			while (bruteMobs.contains(mob))
				bruteMobs.remove(mob);
			return;
		case 3:
			while (eliteMobs.contains(mob))
	...
	/**
	 * This makes sure that the complete set of lists has all of its elements
	 * (levels) set to the appropriate lists.
	 */
	...
	private ArrayList printListDebug(ArrayList list) {/*
														 * System.out.println(list); for(Object o : list) {
	...
	/**
	 * Register the theme with biome types it should and should never appear in.
	 */
	...
	/**
	 * Register a block from the theme with the dungeon block registry.
	 * 
	 * @param block
	 * @return
	 */
	...
	/**
	 * Convert String block names read from the theme file to integer id's
	 * representing the blocks in terms of the dungeon block registry.
	 * 
	 * @param in
	 * @return
	 */
