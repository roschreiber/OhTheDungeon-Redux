...
/**
 * This is the basic room type, built from shape primitive and most common in
 * most dungeon themes. Most of the methods are used for adding features to the
 * room so that it builds itself, though most of these features are actually
 * store in the dungeon map (a MapMatrix) rather than the Room instance.
 * Exception are chests, spawners, and doors, which are kept in lists to be
 * further processes and/or added in ways that accommodate their special
 * features (TileEntities).
 * 
 * Note that the poor encapsulation of this class is largely due to excessive
 * concerns about efficiency and exaggerated assumptions about the overhead of
 * method calls (getters). While this could, and perhaps should be changed in
 * principle, the number of required code changes throughout the mod is now a
 * major reason for leaving them as public.
 * 
...
public class Room extends AbstractRoom {
	...
	/**
	 * A safety method to ensure there are no circular references to create a memory
	 * leak. Note that none should exist, this is included as a safety measure do to
	 * the general complexity of the relationship between dungeons, rooms, and
	 * related rooms.
	 */
	...
	public Room(int beginX, int endX, int beginZ, int endZ, int floorY, int ceilY, Dungeon dungeon, Room parent,
			Room previous) {
		...
		hasEntrance = (isNode && !singleEntrance && dungeon.entrances.use(dungeon.random));
		if (hasEntrance)
	...
	public Room plan(Dungeon dungeon, Room parent) {
		if (!dungeon.complexity.use(dungeon.random) && !isNode) {
			...
			if (dungeon.liquids.use(dungeon.random)) {
				walkway(dungeon);
			} else if (!sky && (dungeon.complexity.use(dungeon.random) || dungeon.symmetry.use(dungeon.random))) {
				cutin(dungeon);
			}
		} else
			addFeatures(dungeon);
	...
	/**
	 * Assigns a wall section between this room and another to the room with the
	 * higher ceiling, so as too avoid overly short walls between rooms of differing
	 * heights.
	 * 
	 * @param dungeon
	 * @param x
	...
	private void assignEdge(Dungeon dungeon, int x, int z) {
		if ((dungeon.map.room[x][z] == 0) || (dungeon.rooms.get(dungeon.map.room[x][z]).sky && !sky) || (isSubroom)) {
			dungeon.map.room[x][z] = id;
			if (!sky)
				dungeon.map.ceiling[x][z] = cielingBlock;
		...
		if (dungeon.map.ceilY[x][z] < (byte) ceilY)
			dungeon.map.ceilY[x][z] = (byte) ceilY;
		if (dungeon.map.nCeilY[x][z] < (byte) ceilY)
			dungeon.map.nCeilY[x][z] = (byte) ceilY;
		if (dungeon.map.floorY[x][z] < (byte) floorY)
			dungeon.map.floorY[x][z] = (byte) floorY;
		if ((dungeon.map.nFloorY[x][z] > (byte) nFloorY) || (dungeon.map.nFloorY[x][z] == 0))
			dungeon.map.nFloorY[x][z] = (byte) floorY;
	}

	/**
	 * Adds features other than chests and spawns to the room to rooms that lack a
	 * whole room pattern. This is called by plan room.
	 * 
	 * It will try to at a number of features based on dungeon complexity. On each
	 * attempt it will check each possible feature once; if a feature is selected to
	 * add it the attempt ends and the next begins. To avoid a universal bias based
	 * on checking order, all features types are added to a list and shuffled once,
	 * giving each room its own set of biases that can be thought of as the room
	 * character or a room equivalent to the dungeons wide theme-based
	 * probabilities.
	 * 
	 * @param dungeon
	 */
	public void addFeatures(Dungeon dungeon) {
		...
		for (int i = 0; i <= tries; i++) {
			for (FeatureAdder feat : features) {
				if (feat.addFeature(dungeon, this))
					break;
			}
		}
	}
	...
	/**
	 * Adds spawners, taking into consideration the mods difficulty setting and
	 * whether the room is an entrance, a destination, or an ordinary room.
	 * 
	 * @param dungeon
	 */
	protected void addSpawners(Dungeon dungeon) {
		...
		if (difficulty == Difficulty.NONE || (!isNode && !difficulty.addmob(dungeon.random))
				|| (hasEntrance && !difficulty.entrancemobs))
			return;
	...
	/**
	 * Checks to see if there are mobs available at the level determined, and if not
	 * adjusts down.
	 * 
	 * @param lev
	 * @param dungeon
	...
	/**
	 * Adds chests, taking into consideration the rooms difficulty and whether its a
	 * destination or ordinary room. No chests will be added to entrance rooms.
	 * 
	 * @param dungeon
	 */
	public void addChests(Dungeon dungeon) {
		...
		if ((difficulty == Difficulty.NONE) || hasEntrance)
			return;
		hasSpawners = spawners.size() > 0;
		if ((!hasSpawners && (dungeon.random.nextInt(5) > 0)))
			return;
		int lev = 0;
		int n = spawners.size();
		for (int i = 0; i < n; i++) {
		...
		if (!hasSpawners) {
			...
			int ms = Math.max(n, 2);
			if (isNode)
				num = Math.min(ms, dungeon.random.nextInt(2 + (ms / 2)) + 2);
			else
				num = dungeon.random.nextInt(1 + (ms / 2)) + 1;
			for (int i = 0; i < num; i++) {
				tmp = (endX - beginX - 3);
				x = dungeon.random.nextInt(tmp) + beginX + 2;
				tmp = (endZ - beginZ - 3);
	...
	public Room connector(Dungeon dungeon, int dir, int xdim, int zdim, int height, Route source) {
		...
		addDoor(dungeon, x, z, (xSeedDir != 0));
		addDoor(dungeon, x + xExtend, z + zExtend, (xSeedDir != 0));
		if (((x + xSeedDir) >= dungeon.size.width) || ((x + xSeedDir) < 0) || ((z + zSeedDir) >= dungeon.size.width)
				|| ((z + zSeedDir) < 0))
			return null;
		if (dungeon.map.room[x + xSeedDir][z + zSeedDir] != 0) {
			return dungeon.rooms.get(dungeon.map.room[x + xSeedDir][z + zSeedDir]);
		} else if ((dir % 2) == 0)
			return new RoomSeed(x + xSeedDir, floorY, z + zSeedDir).growRoomZ(xdim, zdim, height, dungeon, null, this);
		else
			return new RoomSeed(x + xSeedDir, floorY, z + zSeedDir).growRoomX(xdim, zdim, height, dungeon, null, this);
	}

	/**
	 * Fills a room with a "liquid" and then adds a walkway through it; this is used
	 * for rooms with a whole room pattern.
	 * 
	 * @param dungeon
	 */
	private void walkway(Dungeon dungeon) {
		int drop;
		if (dungeon.theme.type.contains(ThemeType.SWAMP))
			drop = 1;
		else
			drop = 2;
		...
		shape.family[orientation].drawWalkway(dungeon, this, realX, realZ, (byte) (endX - beginX + 1),
				(byte) (endZ - beginZ + 1), XFlip, ZFlip);
	}

	/**
	 * Fills the room with walls, then opens up a passage; used for rooms with a
	 * whole room pattern.
	 * 
	 * @param dungeon
	 */
	private void cutin(Dungeon dungeon) {
		...
		shape.family[orientation].drawCutin(dungeon, this, realX, realZ, (byte) (endX - beginX - 1),
				(byte) (endZ - beginZ - 1), XFlip, ZFlip);
	}

	/**
	 * Generate a side room; this is called by Dungeon.growCycle to expand the
	 * dungeon.
	 * 
	 * @param dungeon
	 * @return
...
//			int z = dungeon.random.nextInt(dungeon.size.width);
			int[] range = dungeon.aworld.getUpdatedRange(dungeon.theme.getMinY(), dungeon.theme.getMaxY());
			if (height > range[1])
	...
	/**
	 * Determines the other rooms to which a door leads; this is used in processing
	 * door corrections and room passibility.
	 * 
	 * @param door
	 */
