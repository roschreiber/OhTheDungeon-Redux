...
/**
 * The base type for all rooms, this determines some general room
 * characteristics and provides factory methods for creating rooms the types of
 * which are randomly chosen based on dungeon wide theme-derived variables.
 */
public abstract class AbstractRoom /* extends Shape */ {
	...
	/**
	 * The constructor for the null room (room 0, representing areas that are not
	 * part of the dungeon). It should never be used to generate a "normal" room of
	 * any type. It is only protected (rather than private) because the null room
	 * needs to be instantiated to as a place holder in the room lists.
	 */
	...
	/**
	 * The super constructor for creating real rooms. It is still protected as no
	 * public constructor should exist for rooms as the are provided by the makeRoom
	 * methods.
	 * 
	 * @param dungeon
	 * @param previous
	...
	protected AbstractRoom(Dungeon dungeon, AbstractRoom previous) {
		...
		if (dungeon.degeneracy.value > 0) {
			...
			degenerateFloors = (degenerate && dungeon.degeneracy.use(dungeon.random) && dungeon.random.nextBoolean());
		}
		if (sky)
			fenced = dungeon.fences.use(dungeon.random);
		else
			fenced = false;
	}

	/**
	 * A factory method for creating rooms of a random type. The actual type will be
	 * based on dungeon-wide theme-derived variables.
	 * 
	 * @param beginX
	 * @param endX
	 * @param beginZ
	 ...
	 * @return
	 */
	public static Room makeRoom(int beginX, int endX, int beginZ, int endZ, int floorY, int ceilY, Dungeon dungeon,
			Room parent, Room previous) {
	...
	/**
	 * A factory method for creating rooms of a specified type. Specifying and
	 * unimplemented type will result in a default room of type ROOM / class Room --
	 * that is, a basic room built from shape primitives.
	 * 
	 ...
	 */
	public static Room makeRoom(int beginX, int endX, int beginZ, int endZ, int floorY, int ceilY, Dungeon dungeon,
			Room parent, Room previous, RoomType type) {
		switch (type) {
		case CAVE: {
			Cave base = new Cave(beginX, endX, beginZ, endZ, floorY, ceilY, dungeon, parent, previous);
			return base.plan(dungeon, parent);
