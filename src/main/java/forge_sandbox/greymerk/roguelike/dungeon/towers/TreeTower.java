	...
	private class Branch {
		...
		public Branch(Random rand, Coord start) {
			...
			for (int i = 0; i < mainBranches; ++i) {
				this.branches.add(new Branch(rand, new Coord(end), counter, length, thickness, 4, noise,
						pitch + ((Math.PI * 2 / density) * i), yaw + (rand.nextDouble() - 0.5) * noise));
			}
		}

		public Branch(Random rand, Coord start, int counter, double length, double thickness, int density, double noise,
			...
			for (int i = 0; i < rand.nextInt(density) + 1; ++i) {
				this.branches.add(new Branch(rand, new Coord(end), counter - (rand.nextInt(2) + 1),
						length < 1 ? 1 : length * 0.88, thickness * 0.72, density, noise + (thickness / 5) * 0.55,
						pitch + (rand.nextDouble() - 0.5) * noise, yaw + (rand.nextDouble() - 0.5) * noise));
			}
		}
		...
		private Coord getEnd(Coord start, double length, double pitch, double yaw) {
			...
			Coord offset = new Coord((int) (Math.cos(pitch) * Math.cos(yaw) * length), (int) (Math.sin(yaw) * length),
