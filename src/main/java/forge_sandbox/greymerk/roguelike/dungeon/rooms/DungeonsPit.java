			...
			for (int blockY = originY + dungeonHeight; blockY >= originY - 1; blockY--) {
				for (int blockZ = originZ - dungeonWidth - 1; blockZ <= originZ + dungeonWidth + 1; blockZ++) {
					if (blockX == originX - dungeonLength - 1 || blockZ == originZ - dungeonWidth - 1
							|| blockX == originX + dungeonLength + 1 || blockZ == originZ + dungeonWidth + 1) {
						...
						if (!editor.getMaterial(new Coord(blockX, blockY, blockZ)).isSolid())
							continue;
			...
			for (int z = originZ - 2; z <= originZ + 2; z++) {
				for (int y = originY - 1; y > 0; y--) {
					...
					if (y < 0 + rand.nextInt(5)
							&& editor.getMaterial(new Coord(x, y, z)) == BlockType.get(BlockType.BEDROCK).getBlock()) {
						continue;
					}
