	...
	public boolean generate(IWorldEditor editor, Random rand, LevelSettings settings, Cardinal[] entrances,
		...
		for (Cardinal dir : Cardinal.directions) {
			for (Cardinal orth : Cardinal.orthogonal(dir)) {
				...
				try {
					Treasure.generate(editor, rand, chestPos, types[rand.nextInt(types.length)],
	...
	private static void innerPillars(IWorldEditor editor, Random rand, ITheme theme, int x, int y, int z) {
		...
		for (Cardinal dir : Cardinal.directions) {
			for (Cardinal orth : Cardinal.orthogonal(dir)) {
				...
				pillar.add(dir, 2);
				pillar.add(orth, 2);
				RectSolid.fill(editor, rand, new Coord(pillar.getX(), y - 4, pillar.getZ()),
						new Coord(pillar.getX(), y + 4, pillar.getZ()), secondaryWall);
				pillar.add(dir, 4);
				RectSolid.fill(editor, rand, new Coord(pillar.getX(), y - 4, pillar.getZ()),
						new Coord(pillar.getX(), y + 4, pillar.getZ()), secondaryWall);
				pillar.add(orth, 3);
