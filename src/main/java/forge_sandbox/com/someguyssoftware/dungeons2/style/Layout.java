...
public class Layout {
	...
	/*
	 * this is only here so that a comment can be added to the json without any
	 * special processing
	 */
	...
	public boolean isInteriorComplete() {
		...
		List<DesignElement> enums = EnumSet.allOf(DesignElement.class).stream()
				.filter(x -> (x.getFace() == Face.INTERIOR || x.getFace() == Face.BOTH)).collect(Collectors.toList());

		// compare the lists
		for (DesignElement de : enums) {
			if (!elemNames.contains(de.name()))
				return false;
		}
		return true;
	}
	...
	public enum Type {
		ENTRANCE("entrance"), START("start"), END("end"), ROOM("room"), HALLWAY("hallway"), BOSS("boss"),
		TREASURE("treasure");
