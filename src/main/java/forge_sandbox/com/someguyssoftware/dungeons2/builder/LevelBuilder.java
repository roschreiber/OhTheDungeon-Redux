...
//        
		...
		if (factor < 1.0D) {
			...
			Dungeons2.log.debug("boundary shrunk by -> {} {}, to new size -> {}",
					new Object[] { xAmount, zAmount, newBoundary });
			return newBoundary;
		} else
			return boundary;
	}
	...
	protected List<Room> spawnRooms(Random rand, ICoords startPoint, LevelConfig config) {
		...
		int levelSize = Math.max(MIN_NUMBER_OF_ROOMS, RandomHelper.randomInt(rand,
				config.getNumberOfRooms().getMinInt(), config.getNumberOfRooms().getMaxInt()));

		// generate rooms
		for (int i = 0; i < levelSize; i++) {
			Room room = new Room(i);
			// TODO where should the min width check take place?? - inside the setWidth,
			// setDepth of LevelConfig
			room = randomizeRoom(rand, room, startPoint /* , xOffset, zOffset, */, config);

			// add to the working list that contains all the rooms sorted on distance
			// (farthest to closest)
			rooms.add(room);
		}
	...
	protected List<Room> spawnRooms(Random rand, AxisAlignedBB field, ICoords startPoint, LevelConfig config) {
		...
		for (int i = 0; i < levelSize; i++) {
			...
			// TODO where should the min width check take place?? - inside the setWidth,
			// setDepth of LevelConfig
			// room = randomizeRoom(rand, room, startPoint /*, xOffset, zOffset, */,
			// config);
	...
	protected List<Room> spawnRooms(Random rand, AxisAlignedBB field, ICoords startPoint, ILevelConfig config) {
		...
		int levelSize = Math.max(MIN_NUMBER_OF_ROOMS,
...
//        logger.info("room: " + room);
		...
		room.setCoords(room.getCoords().add(startPoint.getX() - (room.getWidth() / 2), startPoint.getY(),
				startPoint.getZ() - (room.getDepth() / 2)));
	 ...
	 * @return
	 */
	protected Room randomizeRoom(Random rand, Room roomIn, AxisAlignedBB field, ICoords startPoint,
			ILevelConfig config) {
		// randomize dimensions
		Room room = randomizeDimensions(rand, roomIn, config);
...
//        room.getCoords().setY(RandomHelper.randomInt(random, config.getYVariance().getMinInt(), config.getYVariance().getMaxInt()));
	...
	protected Room randomizeDimensions(Random rand, Room roomIn, ILevelConfig config) {
		Room room = new Room(roomIn);
		room.setWidth(Math.max(Room.MIN_WIDTH,
				RandomHelper.randomInt(rand, config.getWidth().getMinInt(), config.getWidth().getMaxInt())));
		room.setDepth(Math.max(Room.MIN_DEPTH,
				RandomHelper.randomInt(rand, config.getDepth().getMinInt(), config.getDepth().getMaxInt())));
		room.setHeight(Math.max(Room.MIN_HEIGHT,
				RandomHelper.randomInt(rand, config.getHeight().getMinInt(), config.getHeight().getMaxInt())));
		return room;
	}
	 ...
	 * @return
	 */
	protected List<Room> applyDistanceBuffering(Random rand, ICoords startPoint, final List<Room> anchors,
			final List<Room> rooms/* , LevelConfig config */) {
		List<Room> bufferedRooms = new ArrayList<>();
		/*
		 * a count of the number times a single room is processed against the list of
		 * buffered rooms
		 */
		int processCount;
...
//        int i = 0;
		/*
		 * process the room against all the rooms in buffered level to see if there is
		 * an overlap
		 */
...
//                    Dungeons2.log.info(String.format("Ignoring... room is flagged as rejected."));
			...
			if (bufferedRooms.size() > 0) {
				...
				bufferedRooms: while (processBufferedRooms) {
...
//                                                                Dungeons2.log.info("\n-------------------\nTesting against processed room: " + bufferedRoom.getId());
						...
						while (roomBB.intersects(bufferedBB)) {
							// TODO need to ensure that an endless loop doesn't occur of a room toggling
							// back and forth
...
//                                    Dungeons2.log.info("Room intersects with processed room:" + roomBB + "; " + bufferedBB);
							...
							if (room.isAnchor()) {
								Dungeons2.log
										.info("Room is anchored. Remove from level as it can not change position.");
								room.setReject(true);
								incrementLossToDistanceBuffering(1);
								continue rooms;
							}
							/*
							 * determine vector from start point. this produces an "explosion" vector moving
							 * away from the start (epicenter). ** seems to take more processing cycles as
							 * there are still overlaps after one round
							 */
...
//                                    double angle = room.getCenter().getXZAngle(bufferedRoom.getCenter());
							 ...
							 * seems to work with minimal cycles NOTE this could process and endless cycle
							 * if the room insects two other rooms which just push back and forth.
							 */
//                                     double angle = room.getCenter().getXZAngle(bufferedRoom.getCenter());
...
//                                    Dungeons2.log.info("intersect:" + intersect);
							...
							/**
							 * Calculate a force value that is equals to the distance from 0 to the amount
							 * of intersection between the two rooms being compared. Add an additional 10%
							 * of force applied to a caridnal directions helps ensure that the amount of
							 * adjustment is more than the amount of intersect.
							 */
							double force = FORCE_SOURCE_COORDS.getDistance(intersect.getX(), 0, intersect.getZ())
									* DEFAULT_FORCE_MODIFIER;
...
//                                            Dungeons2.log.info("Unable to position room... rejecting room.");
								...
								// test against all buffered rooms again to ensure that it doesn't intersect
								// another buffered room now
								// Dungeons2.log.info("No more intersection... testing against all buffered
								// rooms again.");
					...
					/*
					 * flag the endless while loop to stop looping. this is the typical action to
					 * take when all the buffered rooms have been processed.
					 */
...
//        Dungeons2.log.info("BufferedRooms.size=:" + bufferedRooms.size());
	...
	/**
	 * 
	 * @param sourceLevel the originating level to join from. This level owns the
	 *                    shaft that is generated.
	...
	public Shaft join(Level sourceLevel, Level destLevel) {
		...
		List<Room> sourceRooms = sourceLevel.getRooms().stream().filter(room -> room.isStart())
				.collect(Collectors.toList());

		// TODO this line should throw and error
		// check if either list is null
		if (destRooms == null || sourceRooms == null || destRooms.isEmpty() || sourceRooms.isEmpty())
			return shaft;
	...
	public Shaft join(Room sourceRoom, Room destRoom) {
		...
		if (destRoom.getMinY() - sourceRoom.getMaxY() > 1) {
			...
			shaft = (Shaft) new Shaft().setDirection(sourceRoom.getDirection()).setDegrees(0).setType(Type.LADDER);
	 ...
	 * @return
	 */
	public Level build(AsyncWorldEditor world, Random rand, ICoords startPoint, List<Room> plannedRooms,
			ILevelConfig config) {
		...
		// NOTE at this point the level boundary and spawn boundarys should already be
		// set.
		...
		for (Room room : plannedRooms) {
			if (room.isStart() && startRoom == null)
				startRoom = room;
			else if (room.isEnd() && endRoom == null)
				endRoom = room;
			if (room.isAnchor())
				anchors.add(room);
			else
		...
		Dungeons2.log.debug("Before Apply Distance Buffering Rooms.size -> {}",
				new Object[] { anchors.size() + spawned.size() });
		rooms = applyDistanceBuffering(rand, startPoint, anchors, spawned);
		Dungeons2.log.debug("After Apply Distance Buffering Rooms.size -> {}, room loss -> {}",
				new Object[] { rooms.size(), getRoomLossToDistanceBuffering() });
//        System.out.println("After Apply Distance Buffering Rooms.size -> {}, room loss -> {}", rooms.size(), getRoomLossToDistanceBuffering());
		...
		Dungeons2.log.debug("After select valid rooms Rooms.size -> {}, room loss -> {}",
				new Object[] { rooms.size(), getRoomLossToValidation() });
		...
		for (int i = 0; i < rooms.size(); i++) {
			...
			if (rooms.get(i).getMinZ() < mz)
				mz = rooms.get(i).getMinZ();
		}

		// TODO move own method
		// if dimensions are negative, offset all rooms by positive (Math.abs()) amount
		// +1
		if (mx < 0 || mz < 0) {
			for (Room room : rooms) {
		...
		// calculate room waypoints - the coords that build a hallway (edge) between to
		// rooms (vertice)
		waylines = calculateWaylines(rand, paths, rooms);
		if (waylines == EMPTY_WAYLINES)
			return EMPTY_LEVEL;

		// revert room dimensions and generated waylines back to original values by
		// removing offset.
		if (mx < 0 || mz < 0) {
			for (Room room : rooms) {
...
//    }
	...
	private void addDoorsToRoom(Hallway hallway) {
		for (Door d : hallway.getDoors()) {
			// create a new door instance and flip the direction
			Door door = new Door(d.getCoords(), d.getRoom(), d.getHallway(),
					d.getDirection().rotate(Rotate.ROTATE_180));
			d.getRoom().getDoors().add(door);
		}
	}
	...
	protected List<Edge> calculatePaths(Random rand, List<Edge> edges, List<Room> rooms) {
		...
		for (int i = 0; i < addtionalEdges; i++) {
			...
			// TODO ensure that only non-used edges are selected (and doesn't increment the
			// counter)
			Room room1 = rooms.get(e.v);
			Room room2 = rooms.get(e.w);
			if (!room1.isEnd() && !room2.isEnd() && edgeCount[room1.getId()] < room1.getDegrees()
					&& edgeCount[room2.getId()] < room2.getDegrees()) {
				paths.add(e);
				edgeCount[room1.getId()]++;
				edgeCount[room2.getId()]++;
			}
...
//            }
	...
	public Hallway buildHallway(Wayline wayline, List<Room> rooms) {
		...
		if (wayline.getAlignment() == Alignment.HORIZONTAL) {
			...
			/*
			 * update start/end point depending on isTerminal this makes the elbow joint 1
			 * block longer so that they line up correctly
			 */
			if (isElbowJoint) {
			...
			/*
			 * center the hallway: this is to maintain the actual hallway (air part) to
			 * still be along the wayline, since the hallway is 3 wide (2 walls and 1 air)
			 */
		...
		// VERTICAL (NORTH <--> SOUTH)
			...
			// this is to maintain the actual hallway (air part) to still be along the
			// wayline.
			startCoords = startPoint.getCoords();
		...
		int height = Math.abs(Math.min(room1.getMinY(), room2.getMinY()) - Math.max(room1.getMinY(), room2.getMinY()))
				+ 1 + 3; // NOTE why 3? because a doorway is 2 + ceiling block

		// create a temp room out of the dimensions
		Hallway hallway = (Hallway) new Hallway()
				.setCoords(new Coords(startCoords.getX(), startPoint.getCoords().getY(), startCoords.getZ()))
				.setWidth(width).setDepth(depth).setHeight(height).setType(Type.HALLWAY);
		...
		// TODO else { add and set Hallway property. ie this is an elbow join, and this
		// hallway points to another hallway)
		// can't here as it is needs to process the entire wayline first, then produces
		// the hallway
		return hallway;
	}
	...
	/**
	 * perform a breadth first search against the list of edges to determine if a
	 * path exists from one node to another.
	 * 
	 * @param start
	 * @param end
	 * @param rooms
...
//            Dungeons2.log.debug("polling edge id: " + s);
	...
	protected List<Edge> triangulate(List<Room> rooms) {
		/*
		 * maps all rooms by XZ plane (ie x:z) this is required for the Delaunay
		 * Triangulation library because it only returns edges without any identifying
		 * properties, only points
		 */
		Map<String, Room> map = new HashMap<>();
		/*
		 * holds all rooms in Vector2D format. used for the Delaunay Triangulation
		 * library to calculate all the edges between rooms.
		 * 
		 */
...
//            Dungeons2.log.debug(String.format("Room.id: %d = Vector2D: %s", room.getId(), v.toString()));
		...
		for (Triangle2D triangle : triangles) {
			...
			// remove any edges that lead to the end room if the end room already has one
			// edge
			// remove (or don't add) any edges that lead to the end room if the end room
			// already has it's maximum edges (degrees)
...
//            if (!r1.getType().equals(Type.BOSS) && !r2.getType().equals(Type.BOSS)) {
	...
	/**
	 * It is assumed that the rooms list is sorted in some fashion or the caller has
	 * a method to map the matrix indices back to a room object
	 * 
	 * @param rooms
	 * @return
	 */
	 ...
	 * @param config
	 */
	protected List<Wayline> calculateWaylines(Random rand, List<Edge> paths,
			List<Room> rooms/* , LevelConfig config */) {
...
//            midpoint.resetZ(midpoint.getZ()/2);
			...
			/*
			 * a group of maps that relate the room's min/max values with the room NOTE if
			 * it is not necessary to record the ID of the room in the Waypoint, all the
			 * maps are unnecessary as well. UPDATE 8-5-2016 it IS necessary to record the
			 * ID of the room as the Y value of the room needs to be inspected when building
			 * the hallway
			 */
			...
			/*
			 * a stack to contain all the waylines (and sub-waylines) that need to be
...
//            Dungeons2.log.info(String.format("minX: %d, maxX: %d, minZ: %d, maxZ: %d", innerMinX, innerMaxX, innerMinZ, innerMaxZ));
			...
			if ((room1.getMaxZ() <= room2.getMaxZ() && room1.getMaxZ() > (room2.getMinZ() + 1))
				...
				// TODO need some sort of check that if rooms are close by 1 block, which
				// creates a horz wayline that runs from
				// one room fine, but ends in a wall of the second room, which theoretically
				// should be fine, as the door generator
...
//                Dungeons2.log.info(String.format("Vert line from [%d, %d] to [%d, %d[", x, innerMinZ, x, innerMaxZ));
			...
			// TODO special case for <1 lenth rooms.... needs to be a 1 block which would
			// just be the door.
...
//            if (room2.getMinX() + 1 > room1.getMaxX() - 1) {
			if (room2.getCenter().getX() > room1.getCenter().getX()) {
				...
				/*
				 * NOTE 8/31/2017 this comment only concerning rooms where the Y-value is not
				 * the same - which has not been implemented yet.
				 * 
				 * this part is incorrect or just doesn't contain enough data. this is a
				 * L-shaped wayline and therefor point2 doesn't connect to a room by default.
				 * however it is assigned the room2's ID. this will cause incorrect calculations
				 * when attempting to generate hallsways as room2's Y value will be looked at
				 * and not any intermediate intersecting room's Y (the hallway doesn't need to
				 * "climb back" up to room2's Y). needs to be flagged with a value to indicate
				 * that it is not terminal and therefor it can be generated like a corner. as
				 * well need a reference to the next piece of the wayline corner so no overlap
				 * of blocks is generated
				 */
			...
			// if both parts of L-shaped (elbow join) wayline are found, then set to
...
//                Dungeons2.log.info(String.format("Checking against room [%d]", room.getId()));
				...
				AxisAlignedBB bb1 = new AxisAlignedBB(wayline.getPoint1().getX(), 0, wayline.getPoint1().getZ(),
						wayline.getPoint2().getX(), 1, wayline.getPoint2().getZ());

				/*
				 * TODO NOTE encountered a special use case where a wayline appears to connect
				 * start room to end room. the wayline intersects both rooms and enters an
				 * endless loop. this needs to be solved. The wayline that are generated are
				 * 0-length and 1-length. the zero length should be removed. Probably means that
				 * two rooms intersect on a wall and a wayline is build between them. In the
				 * meantime and for good measure in general, add a failsafe
				 */
				// test intersection with room
				if (wayline.getPoint1().getCoords().getDistance(wayline.getPoint2().getCoords()) > 0
						&& bb1.intersects(room.getXZBoundingBox())) {
...
//        if (wayline.getPoint1().getZ() == wayline.getPoint2().getZ()) isHorz = true;
		...
		if (isHorz) {
			...
			if (p1.getX() < p2.getX() /* && wayline.getPoint1() != terminatedPoint */) {
				remainderWayline1 = new Wayline(p1, new Waypoint(room.getId(), room.getMinX(), p1.getY(), p1.getZ()),
						Alignment.HORIZONTAL);
				if (terminatedPoint == null) {
					remainderWayline2 = new Wayline(
							new Waypoint(room.getId(), room.getMaxX() + 1, p2.getY(), p2.getZ(), p2.isTerminated()), p2,
							Alignment.HORIZONTAL);
				}
			}
			// right to left
			else {
				remainderWayline1 = new Wayline(p1,
						new Waypoint(room.getId(), room.getMaxX() + 1, p1.getY(), p1.getZ()), Alignment.HORIZONTAL);
				if (terminatedPoint == null) {
					remainderWayline2 = new Wayline(
							new Waypoint(room.getId(), room.getMinX(), p2.getY(), p2.getZ(), p2.isTerminated()), p2,
							Alignment.HORIZONTAL);
				}
			}
		}
		// vertical
			...
			if (p1.getZ() > p2.getZ()) {
				remainderWayline1 = new Wayline(p1,
						new Waypoint(room.getId(), p1.getX(), p1.getY(), room.getMaxZ() + 1), Alignment.VERTICAL);
				if (terminatedPoint == null)
					remainderWayline2 = new Wayline(
							new Waypoint(room.getId(), p2.getX(), p2.getY(), room.getMinZ(), p2.isTerminated()), p2,
							Alignment.VERTICAL);
			}
			// down
			else {
				remainderWayline1 = new Wayline(p1, new Waypoint(room.getId(), p1.getX(), p1.getY(), room.getMinZ()),
						Alignment.VERTICAL);
				if (terminatedPoint == null)
					remainderWayline2 = new Wayline(
							new Waypoint(room.getId(), p2.getX(), p2.getY(), room.getMaxZ() + 1, p2.isTerminated()), p2,
							Alignment.VERTICAL);
			}
		}
...
//                Dungeons2.log.info("Building wayline (R) from P1 -> Room.MinX");
				if (terminatedPoint == null) {
					remainderWayline = new Wayline(new Waypoint(room.getId(), room.getMaxX(), p2.getY(), p2.getZ()),
							p2);
				}
			} else {
...
//                Dungeons2.log.info("Building wayline (L) from P1 -> Room.MaxX");
				if (terminatedPoint == null) {
					remainderWayline = new Wayline(new Waypoint(room.getId(), room.getMinX(), p2.getY(), p2.getZ()),
							p2);
				}
			}
		}
...
//                Dungeons2.log.info("Skipped V termination point altogether somehow!");
			...
			if (p1.getZ() > p2.getZ()) {
				...
				if (terminatedPoint == null)
					remainderWayline = new Wayline(new Waypoint(room.getId(), p2.getX(), p2.getY(), room.getMinZ()),
							p2);
			}
			// down
			else {
				...
				if (terminatedPoint == null)
					remainderWayline = new Wayline(new Waypoint(room.getId(), p2.getX(), p2.getY(), room.getMaxZ()),
							p2);
			}
		}
	...
	protected List<Room> selectValidRooms(AsyncWorldEditor world, Random rand, List<Room> rooms, ILevelConfig config) {
		...
		for (Room room : rooms) {
			...
			// NOTE at this point it is assumed any anchors are pre-validated and meet all
			// criteria
			if (room.isAnchor()) {
				room.setId(roomId++);
				met.add(room);
			...
			if ((rbb.minX >= lbb.minX && rbb.maxX <= lbb.maxX) && (rbb.minZ >= lbb.minZ && rbb.maxZ <= lbb.maxZ)) {
				isValid = true;
			} else {
				Dungeons2.log.debug("Removing room for being outside level bounds -> {}", new Object[] { room });
//                System.out.println("Removing room for being outside level bounds -> " +  room);
				incrementLossToValidation(1);
			}

			if (isValid && (rbb.minX >= dbb.minX && rbb.maxX <= dbb.maxX)
					&& (rbb.minZ >= dbb.minZ && rbb.maxZ <= dbb.maxZ)) {
				isValid = true;
			} else {
				Dungeons2.log.debug("Removing room for being outside dungeon bounds -> {}",
...
//                    Dungeons2.log.debug("room[{}] is NOT valid at chuck(s) -> {}", room.getId(), room.getXZCenter());
					Dungeons2.log.debug("Removing room for residing in unloaded chunk -> {}",
							new Object[] { room.getId() });
					incrementLossToValidation(1);
				}
			...
			if (isValid) {
				...
				Dungeons2.log.debug("Removing room for failing constraints -> {}", new Object[] { room });
				incrementLossToValidation(1);
			}
	...
	/**
	 * LevelVisualizer against all build criteria. Rooms that don't meet criteria
...
//    }
	...
	protected boolean validateRoomConstraints(AsyncWorldEditor world, Room room, ILevelConfig config) {
		...
		if (room.getCoords().getY() <= getDungeonBuilder().getConfig().getBottomLimit(world.getBottom())) {
			if (Dungeons2.log.isDebugEnabled()) {
				Dungeons2.log.debug("Room bottom [{}] is below min y constraint [{}]", new Object[] {
						room.getCoords().getY(), getDungeonBuilder().getConfig().getBottomLimit(world.getBottom()) });
			}
			return false;
		}
		...
		if (room.getCoords().getY() + room.getHeight() > getDungeonBuilder().getConfig().getTopLimit()) {
			if (Dungeons2.log.isDebugEnabled()) {
				Dungeons2.log.debug(String.format("Room top [%d] is above max y constraint [%d]",
						(room.getCoords().getY() + room.getHeight()), getDungeonBuilder().getConfig().getTopLimit()));
			}
			return false;
		}

		// NOTE these next 3 calls a) take ~1 second to complete and b) cause cascading
		// worldgen lag because they access the world in non-loaded chunks.
...
//        }
		...
		if (room.getCoords().getY() + room.getHeight() > 63) {
			...
			/*
			 * if surfaceRoomDepth is greater than a [x] negative amount. negative implies
			 * the room is higher than the surface, ie the room is exposed.
			 */
...
//    }
	 ...
	 * @return
	 */
	protected Room buildStartRoom(AsyncWorldEditor world, Random rand, AxisAlignedBB field, ICoords startPoint,
			ILevelConfig config) {
		/*
		 * the start of the level
		 */
...
//                        startPoint.getZ()-(startRoom.getDepth()/2)));
		...
		if (!validateRoomConstraints(world, startRoom, config)) {
			Dungeons2.log.debug("Start Room failed room constraints @ " + startRoom.getCenter());
			if (Dungeons2.log.isWarnEnabled()) {
				Dungeons2.log.warn(String.format("Start Room has invalid Minecraft world room conditions: %s",
						startRoom.toString()));
			}
			return EMPTY_ROOM;
		}
...
//    }
	 ...
	 * @return
	 */
	protected Room buildEndRoom(AsyncWorldEditor world, Random rand, AxisAlignedBB field, ICoords startPoint,
			List<Room> plannedRooms, ILevelConfig config) {
		/*
		 * the end room of the level.
		 */

		/*
		 * change the distance that the end room can be from startpoint. (this chance
		 * only affects the end room).
		 */
...
//        c2.setZDistance(qz);
		...
		// build the end room
		Room endRoom = buildPlannedRoom(world, rand, field, startPoint, plannedRooms, config).setEnd(true)
				.setAnchor(true).setType(Type.LADDER);
...
//    }
	 ...
	 * @return
	 */
	protected Room buildPlannedRoom(AsyncWorldEditor world, Random rand, AxisAlignedBB field, ICoords startPoint,
			List<Room> plannedRooms, ILevelConfig config) {
		Room plannedRoom = new Room();

		/*
		 * check to make sure planned rooms don't intersect. test up to 10 times for a
		 * successful position
		 */
		boolean checkRooms = true;
		int endCheckIndex = 0;
...
//    }
	 ...
	 * @return
	 */
	protected Room buildBossRoom(AsyncWorldEditor world, Random rand, AxisAlignedBB field, ICoords startPoint,
			List<Room> predefinedRooms, ILevelConfig config) {
		final int BOSS_ROOM_MIN_XZ = 10;
		final int BOSS_ROOM_MIN_Y = 10;

		Room bossRoom = buildEndRoom(world, rand, field, startPoint, predefinedRooms, config).setType(Type.BOSS)
				.setDegrees(1);
		// ensure min dimensions are met for start room
