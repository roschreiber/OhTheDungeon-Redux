...
//    private static final Logger logger =Bukkit.getLogger();
	 ...
	 */
	@Override
	public Dungeon build(AsyncWorldEditor world, ChunkSnapshot chunk, Random rand, ICoords spawnCoords,
			IDungeonConfig config) {
...
//            this.boundary = dungeonBoundary;
		...
		Dungeons2.log.debug("Dungeon boundary -> {}", new Object[] { dungeonBoundary });
		Dungeons2.log.debug("Dungeon boundary factor -> {}", new Object[] { config.getBoundaryFactor() });
		// resize boundary
		if (config.getBoundaryFactor() > 0D && config.getBoundaryFactor() < 1.0D) {
			int xAmount = (int) (((dungeonBoundary.maxX - dungeonBoundary.minX) * (1.0 - config.getBoundaryFactor()))
					/ 2);
			int zAmount = (int) (((dungeonBoundary.maxZ - dungeonBoundary.minZ) * (1.0 - config.getBoundaryFactor()))
					/ 2);
			dungeonBoundary = dungeonBoundary.grow(-xAmount, 0, -zAmount);
			Dungeons2.log.debug("Dungeon shrunk by factor -> {} [{} {}], to new size -> {}",
					new Object[] { config.getBoundaryFactor(), xAmount, zAmount, dungeonBoundary });
		 ...
		 */
		// TODO resizeBoundary is wrong! 1) it is always using the x-axis and i don't
		// think it is dividing
		AxisAlignedBB lb = getLevelBuilder().resizeBoundary(dungeonBoundary, defaultLevelConfig.getBoundaryFactor());
		Dungeons2.log.debug("init level boundary -> {}, factor -> {}",
				new Object[] { lb, defaultLevelConfig.getBoundaryFactor() });
		AxisAlignedBB roomBoundary = getLevelBuilder().resizeBoundary(lb, defaultLevelConfig.getSpawnBoundaryFactor());
		Dungeons2.log.debug("init spawn boundary -> {}, factor -> {}",
				new Object[] { roomBoundary, defaultLevelConfig.getSpawnBoundaryFactor() });
...
//            startPoint = startPoint.resetY(WorldInfo.getHeightValue(world.getWorld(), startPoint));
		...
		if (this.levelBuilder.getBoundary() == null)
			levelBuilder.setBoundary(dungeonBoundary);
		if (this.levelBuilder.getRoomBoundary() == null)
			levelBuilder.setRoomBoundary(roomBoundary);
		levelBuilder.setDungeonBuilder(this);

		/*
		 * Perform all the minecraft world contraint checks
		 */
		// TODO somehow wrap all these checks into one function. a lot of these
		// variables could be member properties
		...
		if (surfaceCoords.getY() < config.getBottomLimit(world.getBottom())
				|| surfaceCoords.getY() > config.getTopLimit()) {
			Dungeons2.log.debug(String.format(
					"Start position is outside Y constraints: Start %s, yBottom: %d, yTop: %d",
					surfaceCoords.toShortString(), config.getBottomLimit(world.getBottom()), config.getTopLimit()));
			return EMPTY_DUNGEON;
		}

		// 3. determine if startPoint is deep enough to support at least one level
		if (config.isMinecraftConstraints()) {
			if (surfaceCoords.getY() - config.getSurfaceBuffer() - defaultLevelConfig.getHeight().getMax() < config
					.getBottomLimit(world.getBottom())) {
				Dungeons2.log.debug(String.format("Start position is not deep enough to generate a dungeon @ %s",
						surfaceCoords.toShortString()));
				return EMPTY_DUNGEON;
			}
		}
		...
		if (!levelBuilder.isRoomInLoadedChunks(world, entranceRoom)) {
			Dungeons2.log.debug("entrance room is NOT in valid chunks -> {} {}",
					new Object[] { surfaceCoords, surfaceCoords.toChunkPos() });
			return EMPTY_DUNGEON;
		}

		// 6. ensure that the entrance has a valid base
		// boolean flag = !WorldInfo.isValidAboveGroundBase(world,
		// entranceRoom.getCoords().resetY(surfaceCoords.getY()),
		// entranceRoom.getWidth(), entranceRoom.getDepth(), 50, 20, 50);
		boolean flag = false;
		if (config.isMinecraftConstraints() && flag) {
			if (Dungeons2.log.isDebugEnabled())
				Dungeons2.log
						.debug(String.format("Surface area does not meet ground/air criteria @ %s", surfaceCoords));
			return EMPTY_DUNGEON;
		}
		...
		if (startPoint.getY() > world.getSeaLevel()) {
			// offset for mountains
			int diff = startPoint.getY() - world.getSeaLevel();
			startPoint = startPoint.add(0, -diff, 0);
		}
		 ...
		 */
		int numberOfLevels = RandomHelper.randomInt(rand, (int) config.getNumLevels().getMin(),
				(int) config.getNumLevels().getMax());
...
//            logger.debug("Building level -> {}", levelIndex);
			...
			Dungeons2.log.debug("using config -> {}", new Object[] { levelConfig });

			// update the level/roomBoundary if the level config has changed
			if (levelConfig != prevLevelConfig) {
				// config has changed
				AxisAlignedBB levelBoundary = getLevelBuilder().resizeBoundary(dungeonBoundary,
						levelConfig.getBoundaryFactor());
				getLevelBuilder().setBoundary(levelBoundary);
				Dungeons2.log.debug("new level boundary ->{} (factor -> {}",
						new Object[] { levelBoundary, levelConfig.getBoundaryFactor() });
				...
				Dungeons2.log.debug("new spawn boundary ->{} (factor -> {}",
						new Object[] { roomBoundary, levelConfig.getSpawnBoundaryFactor() });
			}

			// determine if any levels can be made below this one
			if (startPoint.getY() - levelConfig.getHeight().getMax() < config.getBottomLimit(world.getBottom())) {
				isBottomLevel = true;
			}
...
//                    logger.log(java.util.logging.Level.WARNING, "Unable to generate level Start Room: " + levelIndex);
				...
				// 2. create end room
				endRoom = levelBuilder.buildPlannedRoom(world, rand, roomBoundary, startPoint, plannedRooms,
						levelConfig);
				if (endRoom == LevelBuilder.EMPTY_ROOM) {
...
//            logger.log(java.util.logging.Level.WARNING, "Unable to build dungeon. 0 levels were built.");
	 ...
	 * 
	 * @see
	 * com.someguyssoftware.dungeons2.builder.IDungeonBuilder#build(net.minecraft.
	 * world.World, java.util.Random, com.someguyssoftware.mod.ICoords,
	 * com.someguyssoftware.dungeons2.model.DungeonConfig)
	 */
//    @Override
//    @Deprecated
...
//    }
	...
	private AxisAlignedBB getRoomBoundary(AxisAlignedBB boundary) {
		 ...
		 */
		Vec3d center = new Vec3d(boundary.minX + (boundary.maxX - boundary.minX) * 0.5D,
				boundary.minY + (boundary.maxY - boundary.minY) * 0.5D,
				boundary.minZ + (boundary.maxZ - boundary.minZ) * 0.5D);
	...
	private AxisAlignedBB getDungeonBoundary(ICoords spawnCoords, ICoords closestCoords) {
		...
		Dungeons2.log.debug("spawnCoords -> {}", new Object[] { spawnCoords });
		Dungeons2.log.debug("deltaCoords -> {}", new Object[] { deltaCoords.toShortString() });
		// get max. distance of the axises, ensuring a min/max size is met
		int dist = Math.max(Math.min(Math.max(Math.abs(deltaCoords.getX()), Math.abs(deltaCoords.getZ())), 256), // 256
																													// =
																													// MAX_FIELD_RADIUS
				25); // 25 = MIN_FIELD_RADIUS
		Dungeons2.log.debug("dist from player to spawn -> {}", new Object[] { dist });
		...
		if (Math.abs(deltaCoords.getX()) >= Math.abs(deltaCoords.getZ())) {
			Dungeons2.log.debug("deltaX -> {} >= deltaZ -> {}",
					new Object[] { Math.abs(deltaCoords.getX()), Math.abs(deltaCoords.getZ()) });
			// WEST/EAST
	...
	public Room buildEntranceRoom(AsyncWorldEditor world, Random rand, ICoords startPoint) {
		...
		/*
		 * adjust the minimum dimension sizes for entrance room. entrances will always
		 * be a square-based (xz axis) room and odd numbered (ex length/width=5)
		 */
		...
		room.setCoords(new Coords(startPoint.getX() - ((room.getWidth() - 1) / 2), startPoint.getY(),
	 ...
	 * 
	 * @see
	 * com.someguyssoftware.dungeons2.builder.IDungeonBuilder#setLevelBuilder(com.
	 * someguyssoftware.dungeons2.builder.LevelBuilder)
	 */
	@Override
