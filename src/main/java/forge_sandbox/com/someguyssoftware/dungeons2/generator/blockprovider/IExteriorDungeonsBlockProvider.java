...
public interface IExteriorDungeonsBlockProvider extends IDungeonsBlockProvider {
	...
	default public boolean isCrownElement(ICoords coords, Room room, Layout layout) {
		...
		return coords.getY() == room.getMaxY() + yOffset
				&& (((coords.getX() == room.getMinX() + xzOffset || coords.getX() == room.getMaxX() - xzOffset)
						...
						|| ((coords.getZ() == room.getMinZ() + xzOffset || coords.getZ() == room.getMaxZ() - xzOffset)
								&& coords.getX() >= room.getMinX() + xzOffset
								&& coords.getX() <= room.getMaxX() - xzOffset));
	}
	...
	default public boolean isTrimElement(ICoords coords, Room room, Layout layout) {
		...
		if (room.hasPlinth() || room.hasColumn()) { // TODO check for hasInsetWall
			 ...
			 */
					room.getWidth() >= 9 && room.getDepth() >= 9 && room.getHeight() > 4 && y == room.getMinY() + 1
					&& (((x == room.getMinX() + 2 || x == room.getMaxX() - 2) && z > room.getMinZ()
							&& z < room.getMaxZ())
							|| ((z == room.getMinZ() + 2 || z == room.getMaxZ() - 2) && x > room.getMinX()
									&& x < room.getMaxX())))
				return true;
		} else {
	...
	default public boolean isPlinthElement(ICoords coords, Room room, Layout layout) {
		...
		if (coords.getY() == room.getMinY()) {
			if (coords.getX() == room.getMinX() || coords.getX() == room.getMaxX() || coords.getZ() == room.getMinZ()
	...
	default public boolean isCrenellationElement(ICoords coords, Room room, Layout layout) {
		if (coords.getY() >= room.getMaxY() - 2 && (coords.getX() == room.getMinX() || coords.getX() == room.getMaxX()
				|| coords.getZ() == room.getMinZ() || coords.getZ() == room.getMaxZ()))
			return true;
		return false;
	}
	...
	/**
	 * Merlons occur at each of the four corners, immediately adjacent to the
	 * corners and then every other block.
	 * 
	 * @param coords
	 * @param room
	 * @param layout
		...
		if ((room.hasCrenellation() || room.hasMerlon()) && y == room.getMaxY()) {
			...
			if (room.getWidth() > 5) {
				if ((x == room.getMinX() + 1 || x == room.getMaxX() - 1)
						&& (z == room.getMinZ() || z == room.getMaxZ()))
					return true;
				if ((z == room.getMinZ() + 1 || z == room.getMaxZ() - 1)
						&& (x == room.getMinX() || x == room.getMaxX()))
					return true;

				// every other block in the middle
				if (((x == room.getMinX() || x == room.getMaxX()) && Math.abs(zIndex % 2) == 1 && zIndex > 0
						&& zIndex < room.getDepth() - 1)
						|| ((z == room.getMinZ() || z == room.getMaxZ()) && Math.abs(xIndex % 2) == 1 && xIndex > 0
	...
	default public boolean isInsetWallElement(ICoords coords, Room room, Layout layout) {
		/*
		 * NOTE remember to minus an additional 1 for the ceiling so crenellation
		 * becomes -3, merlon || parapet becomes -2
		 */
		int x = coords.getX();
		int y = coords.getY();
		int z = coords.getZ();
	...
	default public boolean isWindowElement(ICoords coords, Room room, Layout layout) {
		...
		/*
		 * very specific windows on either side of door for small entrances, 1 block
		 * above door
		 */
		if (room.getWidth() == 5) {
			if ((xIndex == 1 || xIndex == 3 || zIndex == 1 || zIndex == 3) && (yIndex > 2 && yIndex < 5)) {
//                Dungeons2.log.debug("Has Window!");
				return true;
			}
		} else if (room.getWidth() == 7) {
			if ((xIndex == 2 || xIndex == 4 || zIndex == 2 || zIndex == 4) && (yIndex > 2 && yIndex < 5)) {
//                Dungeons2.log.debug("Has Window");
				return true;
			}
		}
	...
	default public boolean isColumnElement(ICoords coords, Room room, Layout layout) {
		...
		return ((x == room.getMinX() || x == room.getMaxX()) && Math.abs(zIndex % 2) == 0 && zIndex > 0
				&& zIndex < room.getDepth() - 1)
				|| ((z == room.getMinZ() || z == room.getMaxZ()) && Math.abs(xIndex % 2) == 0 && xIndex > 0
						&& xIndex < room.getWidth() - 1);
	}
