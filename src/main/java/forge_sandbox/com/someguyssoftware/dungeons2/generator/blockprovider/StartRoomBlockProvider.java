...
public class StartRoomBlockProvider implements IDungeonsBlockProvider {
	...
	public DesignElement getDesignElement(ICoords coords, Room room, Layout layout) {
		...
		if (isLadderPillarElement(coords, room, layout))
			return DesignElement.LADDER_PILLAR;
		if (isLadderElement(coords, room, layout))
			return DesignElement.LADDER;

		/*
		 * pillars should only be near corners ie only 4 pillars near corners - not
	...
	// TODO move to AbstractAdvanced or AbstractSpecial RoomGenerator --> actually
	// to AbstractStartRoomGenerator
	/**
	 * 
	 * @param coords
	 * @param room
	...
	public boolean isLadderPillarElement(ICoords coords, Room room, Layout layout) {
		...
		if (coords.getX() == center.getX() && coords.getZ() == center.getZ() && coords.getY() > room.getMinY())
			return true;
		return false;
	}
	...
	public boolean isLadderElement(ICoords coords, Room room, Layout layout) {
		...
		switch (direction) {
		case NORTH:
			if (coords.getX() == center.getX() && coords.getZ() == (center.getZ() + 1)
					&& coords.getY() > room.getMinY())
				return true;
			break;
		case EAST:
			if (coords.getX() == (center.getX() - 1) && coords.getZ() == center.getZ()
					&& coords.getY() > room.getMinY())
				return true;
			break;
		case SOUTH:
	...
	public boolean isPillarElement(ICoords coords, Room room, Layout layout) {
		...
		if ((x == room.getMinX() + 2 || x == room.getMaxX() - 2)
				&& (z == room.getMinZ() + 2 || z == room.getMaxZ() - 2))
			return true;
		return false;
	}
}
