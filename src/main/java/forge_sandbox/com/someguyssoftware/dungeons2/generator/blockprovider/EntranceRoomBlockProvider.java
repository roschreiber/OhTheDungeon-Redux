...
public class EntranceRoomBlockProvider implements IExteriorDungeonsBlockProvider {
	...
	public DesignElement getDesignElement(ICoords coords, Room room, Layout layout) {
		// TODO make ceiling height a property of Room, then wouldn't have to
		// recalculate this for every block
...
//        if (isLadderPillarElement(x, y, z, room, layout)) return DesignElement.LADDER_PILLAR;
		if (isLadderElement(coords, room, layout))
...
//                return DesignElement.WALL;
			...
			// build plinths and cornices before because they will get replaced/overwritten
			// by columns
			if (isPlinthElement(coords, room, layout))
				element = DesignElement.PLINTH;
			else if (isCorniceElement(coords, room, layout))
...
//                Dungeons2.log.debug("Is Column Element @ " + coords.toShortString());
			...
			// TODO should return if any part of the window, then check if base, window, or
			// capital
			// check for window
			if (isWindowElement(coords, room, layout))
		...
		// check for trim
		if (room.hasTrim() && isTrimElement(coords, room, layout))
		...
		if (room.hasCrenellation()) {
			if (isCrenellationElement(coords, room, layout)) {
				if (isParapetElement(coords, room, layout))
					return DesignElement.PARAPET;
				if (isMerlonElement(coords, room, layout))
					return DesignElement.MERLON;
			}
		} else if (room.hasMerlon() && isMerlonElement(coords, room, layout))
			return DesignElement.MERLON;
		else if (room.hasParapet() && isParapetElement(coords, room, layout))
			return DesignElement.PARAPET;
	...
	public boolean isFacadeSupport(ICoords coords, Room room, Layout layout) {
		...
		return coords.getY() == h || coords.getY() == h - 1 || coords.getY() == getCorniceHeight(room, layout);
	}
