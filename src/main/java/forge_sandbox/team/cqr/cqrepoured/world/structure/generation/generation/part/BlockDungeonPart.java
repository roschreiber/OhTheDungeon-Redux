...
public class BlockDungeonPart implements IDungeonPart, IProtectable {
	...
	public static class GeneratableChunkInfo implements IGeneratable {
		...
		@Override
		public void generate(AsyncWorldEditor world, GeneratableDungeon dungeon) {
			BlockPlacingHelper.setBlockStates(world, dungeon, (world1, dungeon1) -> {
				boolean flag = false;
				for (GeneratablePosInfo generatable : this.blocks) {
					flag |= generatable.place(world1, dungeon1);
				}
				if (flag) {
	...
	public static class Builder implements IDungeonPartBuilder {
		...
		@Override
		public BlockDungeonPart build(AsyncWorldEditor world, DungeonPlacement placement) {
			List<GeneratablePosInfo> list = this.blocks.stream().map(preparable -> preparable.prepare(world, placement))
			...
			for (int i = 0; i < list.size(); i++) {
				...
				while (i < list.size() - 1) {
					...
					if (first.getChunkX() != next.getChunkX() || first.getChunkY() != next.getChunkY()
							|| first.getChunkZ() != next.getChunkZ()) {
						break;
					}
					list2.add(next);
					i++;
