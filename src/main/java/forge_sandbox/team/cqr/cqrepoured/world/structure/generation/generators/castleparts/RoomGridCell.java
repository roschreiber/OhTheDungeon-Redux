...
public class RoomGridCell {
	...
	private Map<BlockFace, CastleMainStructWall> walls = new EnumMap<>(BlockFace.class);
	...
	public void registerAdjacentWall(CastleMainStructWall wall, BlockFace directionOfWall) {
		this.walls.put(directionOfWall, wall);
	}

	public Optional<CastleMainStructWall> getAdjacentWall(BlockFace direction) {
	...
	public void setAllLinkedReachable(List<RoomGridCell> unreachableCells, List<RoomGridCell> reachableCells) {
		...
		for (RoomGridCell linkedCell : this.getConnectedCellsCopy()) {
			...
			// TODOo: This would be easier and faster with a hashset
	...
	// Returns true if this room is selected to build but has not been populated
	// with a room
	...
	public void addDoorOnSideCentered(BlockFace side, EnumCastleDoorType type, Random random) {
		if (this.walls.containsKey(side)) {
			this.walls.get(side).addDoorCentered(type, random);
		}
	}

	public void addDoorOnSideRandomOffset(BlockFace side, EnumCastleDoorType type, Random random) {
		if (this.walls.containsKey(side)) {
			this.walls.get(side).addDoorRandomOffset(type, random);
		}
	}

	public void addOuterWall(BlockFace side) {
		if (this.walls.containsKey(side)) {
	...
	public void addRoofEdgeWall(BlockFace side) {
		if (this.walls.containsKey(side)) {
			this.walls.get(side).enable();
			this.walls.get(side).setAsOuterWall();
	...
	public void addInnerWall(BlockFace side) {
		if (this.walls.containsKey(side)) {
			this.walls.get(side).enable();
	...
	public void removeWall(BlockFace side) {
		if (this.walls.containsKey(side)) {
	...
	public void copyRoomPropertiesToConnectedCells() {
		List<CastleRoomBase> connectedRooms = this.connectedCells.stream().filter(RoomGridCell::isPopulated)
				.map(RoomGridCell::getRoom).collect(Collectors.toList());
		for (CastleRoomBase connectedRoom : connectedRooms) {
			connectedRoom.copyPropertiesOf(this.room);
		}
	}

	public List<BlockFace> getPotentialBridgeDirections() {
	...
	public boolean isValidForBridge() {
		if (this.isNotSelected()) {
			Optional<RoomGridCell> below = this.getAdjacentCell(BlockFace.DOWN);
	...
	public String toString() {
		...
		return String.format("RoomGridCell{%s, state=%s, room=%s}", this.gridPosition.toString(), this.state.toString(),
				roomStr);
	}
	...
	@Override
	public int hashCode() {
		// Use just the gridPosition as a hash so we can keep sets of cells with only
