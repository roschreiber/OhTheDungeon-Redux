...
public class RoomGrid {
	public static class Area2D {
		...
		public Area2D addFloors(int numFloors) {
			return new Area2D(this.start.move(BlockFace.UP, numFloors), this.sizeX, this.sizeZ);
		}
		...
		public List<RoomGridPosition> getPositionList() {
			...
			for (int x = 0; x < this.sizeX; x++) {
				for (int z = 0; z < this.sizeZ; z++) {
					positions.add(this.start.move(BlockFace.EAST, x).move(BlockFace.SOUTH, z));
				}
			}
		...
		public Area2D getRandomSubArea(Random random, int minDim1, int minDim2, boolean mustBeSmaller) {
			...
			if (this.dimensionsAreAtLeast(minDim1, minDim2)) {
				...
				// Determine which directions (X and Z) have the room to fit the longer of the
				// two dimensions
		...
		public Area2D getExactSubArea(Random random, int minDim1, int minDim2) {
			...
			if (this.dimensionsAreAtLeast(minDim1, minDim2)) {
				...
				// If either dimension could be the long side, then pick at random, otherwise
				// force long side
		...
		private Area2D randomlyPositionSubArea(Random random, int resultX, int resultZ) {
			...
			if (maxMoveX > 0) {
				subStart = subStart.move(BlockFace.EAST, random.nextInt(this.sizeX - resultX));
			}
			if (maxMoveZ > 0) {
				subStart = subStart.move(BlockFace.SOUTH, random.nextInt(this.sizeZ - resultZ));
			}
		...
		@Nullable
		public Area2D sliceToSideOfArea(Area2D mask, BlockFace side) {
			if (mask != null) {
		...
		public void alignToSide(Random random, Area2D targetArea, BlockFace side, Area2D boundary) {
//			int distance; // distance to move toward target area so we are adjacent

			// slide position is position perpendicular to the side to are least 1 square is
			// touching
			int minSlide;
			int maxSlide;
			int slideDest;
				...
				for (int z = 0; z < this.roomsZ; z++) {
					...
					for (BlockFace direction : BlockFace.values()) {
						...
						if (adjacent != null) {
							...
							adjacent.registerAdjacentCell(cell, direction.getOppositeFace());
						}
					}
				}
			}
		...
		for (int floor = 0; floor < this.floors; floor++) {
			for (int x = 0; x < this.roomsX + 1; x++) {
				for (int z = 0; z < this.roomsZ; z++) {
					...
					CastleMainStructWall wall = new CastleMainStructWall(wallOrigin,
							CastleMainStructWall.WallOrientation.VERTICAL, wallWidth, floorHeight);
					this.wallList.add(wall);

					RoomGridCell westCell = this.getCellAt(floor, x - 1, z);
					if (westCell != null) {
						wall.registerAdjacentCell(westCell, BlockFace.WEST);
						westCell.registerAdjacentWall(wall, BlockFace.EAST);
					}

					RoomGridCell eastCell = this.getCellAt(floor, x, z);
					if (eastCell != null) {
						wall.registerAdjacentCell(eastCell, BlockFace.EAST);
						eastCell.registerAdjacentWall(wall, BlockFace.WEST);
					}
				}
			}
		}
		...
		for (int floor = 0; floor < this.floors; floor++) {
			for (int x = 0; x < this.roomsX; x++) {
				for (int z = 0; z < this.roomsZ + 1; z++) {
					...
					BlockPos wallOrigin = new BlockPos(xOffset, yOffset, zOffset);
					CastleMainStructWall wall = new CastleMainStructWall(wallOrigin,
							CastleMainStructWall.WallOrientation.HORIZONTAL, wallWidth, floorHeight);
					this.wallList.add(wall);

					RoomGridCell northCell = this.getCellAt(floor, x, z - 1);
					if (northCell != null) {
						wall.registerAdjacentCell(northCell, BlockFace.NORTH);
						northCell.registerAdjacentWall(wall, BlockFace.SOUTH);
					}

					RoomGridCell southCell = this.getCellAt(floor, x, z);
					if (southCell != null) {
						wall.registerAdjacentCell(southCell, BlockFace.SOUTH);
						southCell.registerAdjacentWall(wall, BlockFace.NORTH);
					}
				}
			}
		}
	...
	/*
	 * Returns a list (from largest area to smallest area) of contiguous 2d grid
	 * areas that are on a given floor and satisfy a given condition.
	 *
	 * Note: Areas 2x2 or larger will always evaluate as larger than anything 1xN or
	 * Nx1. This is done on purpose because 2x2+ areas are more useful for building.
	 */
	public List<Area2D> getAllGridAreasWhere(int floor, Predicate<RoomGridCell> condition, int minDimension1,
			int minDimension2) {
		...
		while (largest != null && !floorPositions.isEmpty()
				&& largest.dimensionsAreAtLeast(minDimension1, minDimension2)) {
			areas.add(largest);
	...
	public Area2D getPotentialRoomBuildArea(RoomGridPosition rootPosition) {
		...
		do {
			...
			for (int i = 0; i < z; i++) {
				RoomGridPosition checkPos = rootPosition.move(BlockFace.EAST, (x - 1)).move(BlockFace.SOUTH, i);
			...
			for (int i = 0; i < x; i++) {
				RoomGridPosition checkPos = rootPosition.move(BlockFace.EAST, i).move(BlockFace.SOUTH, (z - 1));
				if ((this.getCellAt(checkPos) == null) || (!this.getCellAt(checkPos).needsRoomType())) {
					incZ = false;
					--z;
					break;
		...
		if (!floorPositions.isEmpty()) {
			for (RoomGridPosition startPos : floorPositions) {
				...
				do {
					...
					for (int i = 0; i < z; i++) {
						RoomGridPosition checkPos = startPos.move(BlockFace.EAST, (x - 1)).move(BlockFace.SOUTH, i);
						if (!floorPositions.contains(checkPos) || !this.withinGridBounds(checkPos)
								|| !condition.test(this.getCellAt(checkPos))) {
							incX = false;
							--x;
							break;
						}
					...
					for (int i = 0; i < x; i++) {
						RoomGridPosition checkPos = startPos.move(BlockFace.EAST, i).move(BlockFace.SOUTH, (z - 1));
						if (!floorPositions.contains(checkPos) || !this.withinGridBounds(checkPos)
								|| !condition.test(this.getCellAt(checkPos))) {
							incZ = false;
							--z;
	...
	public void selectBlockOfCellsForBuilding(Area2D area, int numFloors) {
		...
		for (RoomGridPosition areaPos : positions) {
			...
			for (; floor < numFloors; floor++) {
				gridPos = areaPos.move(BlockFace.UP, floor);
	...
	private void setPathingForCellSet(Set<RoomGridCell> cellsInArea) {
		...
		for (RoomGridCell cell : cellsInArea) {
			...
			// Check each horizontal (BlockFace order is S W N E)
		...
		// Reflexively copy out the new master list so all connected sections path to
		// each other
		// This MUST be done last, since we don't know who is around the new cell area
		// until we check all of them
		for (RoomGridCell cell : masterPathList) {
			cell.addPathableCells(masterPathList);
		}
	}

	public boolean adjacentCellIsPopulated(RoomGridCell startCell, BlockFace direction) {
		RoomGridCell adjacent = this.getAdjacentCell(startCell, direction);
		return (adjacent != null && adjacent.isPopulated());
	}

	public boolean adjacentCellIsFullRoom(RoomGridCell startCell, BlockFace direction) {
		RoomGridCell adjacent = this.getAdjacentCell(startCell, direction);
		return (adjacent != null && adjacent.isPopulated() && !(adjacent.getRoom() instanceof CastleRoomWalkableRoof));
	}

	public boolean adjacentCellIsSelected(RoomGridCell startCell, BlockFace direction) {
		RoomGridCell adjacent = this.getAdjacentCell(startCell, direction);
		return (adjacent != null && adjacent.isSelectedForBuilding());
	}

	public boolean adjacentCellIsWalkableRoof(RoomGridCell startCell, BlockFace direction) {
		RoomGridCell adjacent = this.getAdjacentCell(startCell, direction);
		return (adjacent != null && adjacent.isPopulated() && adjacent.getRoom() instanceof CastleRoomWalkableRoof);
	}

	public boolean cellIsValidForRoof(RoomGridCell cell) {
		RoomGridCell below = this.getAdjacentCell(cell, BlockFace.DOWN);

		return (below != null && !cell.isSelectedForBuilding() && below.isPopulated()
				&& (below.getFloor() != this.bossArea.start.getFloor())); // Don't want
																			// to build
																			// roofs
																			// over the
																			// boss
	...
	public boolean cellIsOuterEdge(RoomGridCell cell, BlockFace direction) {
	...
	public List<RoomGridCell> getBridgeCells(RoomGridCell cell, BlockFace direction) {
		...
		if (!next.isPresent()) {
			...
			result.clear();
		} else if (next.get().isPopulated() && !next.get().reachableFromSide(direction.getOppositeFace())) {
	...
	public List<RoomGridCell> getAdjacentSelectedCellsInRow(RoomGridPosition position) {
		...
		while (this.getCellAt(position.move(BlockFace.WEST)) != null
				&& this.getCellAt(position.move(BlockFace.WEST)).isSelectedForBuilding()) {
			position = position.move(BlockFace.WEST);
		}

		result.add(this.getCellAt(position));

		while (this.getCellAt(position.move(BlockFace.EAST)) != null
				&& this.getCellAt(position.move(BlockFace.EAST)).isSelectedForBuilding()) {
			position = position.move(BlockFace.EAST);
			result.add(this.getCellAt(position));
		}
	...
	public List<RoomGridCell> getAdjacentSelectedCellsInColumn(RoomGridPosition position) {
		...
		while (this.getCellAt(position.move(BlockFace.NORTH)) != null
				&& this.getCellAt(position.move(BlockFace.NORTH)).isSelectedForBuilding()) {
			position = position.move(BlockFace.NORTH);
		}

		result.add(this.getCellAt(position));

		while (this.getCellAt(position.move(BlockFace.SOUTH)) != null
				&& this.getCellAt(position.move(BlockFace.SOUTH)).isSelectedForBuilding()) {
			position = position.move(BlockFace.SOUTH);
			result.add(this.getCellAt(position));
		}
	 ...
	 */
	public boolean canAttachTower(RoomGridCell cell, BlockFace side) {
		RoomGridCell adjacent = this.getAdjacentCell(cell, side);

		return (!cell.getRoom().isTower() && adjacent != null
				&& (!adjacent.isPopulated() || cell.getRoom().isStairsOrLanding()));
	}
	...
	@Nullable
	public RoomGridCell getAdjacentCell(RoomGridCell startCell, BlockFace direction) {
	...
	public boolean withinGridBounds(RoomGridPosition position) {
		return (position.getFloor() >= 0 && position.getFloor() < this.floors
				&& this.withinFloorBounds(position.getX(), position.getZ()));
	}
