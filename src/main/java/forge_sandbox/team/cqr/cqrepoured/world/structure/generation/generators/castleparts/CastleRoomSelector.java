...
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.addons.CastleAddonRoofBase;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.addons.CastleRoofFactory;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomBase;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomBossLandingEmpty;
...
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomBossStairEmpty;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomBossStairMain;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomBridgeTop;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomHallway;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomJailCell;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomLandingDirected;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomLandingDirectedBoss;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomLandingSpiral;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomReplacedRoof;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomRoofBossEmpty;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomRoofBossMain;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomStaircaseDirected;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomStaircaseSpiral;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomTowerSquare;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomWalkableRoof;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.CastleRoomWalkableRoofTower;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.EnumRoomType;
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.RoomFactoryCastle;
...
import forge_sandbox.team.cqr.cqrepoured.world.structure.generation.generators.castleparts.rooms.segments.EnumCastleDoorType;
...
/**
 * Copyright (c) 20.12.2019 Developed by KalgogSmash GitHub:
...
public class CastleRoomSelector {
	...
	public CastleRoomSelector(DungeonRandomizedCastle dungeon, Random rand) {
		...
		// Add padding floors so that we can build walkable roofs on top of the highest
		// rooms
		this.grid = new RoomGrid(this.maxFloors + PADDING_FLOORS, gridSizeX, gridSizeZ, this.roomSize, this.floorHeight,
				this.random);
	}
	...
	public void generate(AsyncWorldEditor world, BlockStateGenArray genArray, DungeonRandomizedCastle dungeon,
			BlockPos startPos, DungeonInhabitant mobType) {
		this.generateRooms(startPos, dungeon, genArray);
		this.generateWalls(genArray, dungeon);

		this.addDecoration(world, startPos, dungeon, genArray, mobType);

		this.generateRoofs(startPos, genArray, dungeon);
	}

	private void generateRooms(BlockPos startPos, DungeonRandomizedCastle dungeon, BlockStateGenArray genArray) {
		// Generate everything except walkable roofs. Walkable roofs should be done at
		// the very end
		// because they have the lowest block priority (all other parts should
		// overwrite)
		for (RoomGridCell cell : this.grid
				.getAllCellsWhere(c -> (c.isPopulated()) && !(c.getRoom() instanceof CastleRoomWalkableRoof))) {
	...
	private void addDecoration(AsyncWorldEditor world, BlockPos startPos, DungeonRandomizedCastle dungeon,
			BlockStateGenArray genArray, DungeonInhabitant mobType) {
		EntityType mobResLoc = mobType.getEntityID();
		BossInfo bossInfo = mobType.getBossInfo();
		...
		for (RoomGridCell cell : this.grid.getAllCellsWhere(RoomGridCell::isPopulated)) {
			...
			cell.getRoom().placeBoss(world, genArray, dungeon, bossInfo);
	...
	private DungeonInhabitant selectJailInhabitant(AsyncWorldEditor world, DungeonInhabitant mainInhabitant) {
		return DungeonInhabitantManager.instance().getRandomInhabitant();
		// Faction inhaFaction;
...
//
//			// It is possible for entity to be null here but getFactionOf handles that case
...
//			List<DungeonInhabitant> possibleJailed = DungeonInhabitantManager.instance()
//					.getListOfFactionInhabitants(enemyFaction, world);
...
//		return jailed;
	...
	private void createCastleLayout() {
		...
		for (layer = 0; ((!lastFloor) && (layer < MAX_LAYERS)); layer++) {
			...
			List<RoomGrid.Area2D> buildableAreas = this.grid.getAllGridAreasWhere(firstFloorInLayer,
					RoomGridCell::isBuildable, 2, 2);
			...
			if (!buildableAreas.isEmpty()) {
				for (RoomGrid.Area2D buildArea : buildableAreas) {
					...
					if (buildableAreas.get(0) == buildArea) {
						if (buildArea.dimensionsAreAtLeast(this.minRoomsForBoss, this.minRoomsForBoss + 1)) {
							if (buildArea.dimensionsAre(this.minRoomsForBoss, this.minRoomsForBoss + 1)) {
								...
								// CQRMain.logger.info("At minimum boss area so setting boss area to: {}",
								// buildArea);
								...
								if (layer >= 3) {
									RoomGrid.Area2D bossArea = buildArea.getExactSubArea(this.random,
											this.minRoomsForBoss, this.minRoomsForBoss + 1);
									// grid.selectBlockOfCellsForBuilding(bossArea, floorsPerLayer);
									// CQRMain.logger.info("At high enough layer so setting boss area to: {}",
									// bossArea);
									this.grid.setBossArea(bossArea);
									lastFloor = true;

									// TODOo: Make use of any remaining space by subtracting boss area from
									// buildarea
								} else {
									RoomGrid.Area2D structArea = buildArea.getRandomSubArea(this.random,
											this.minRoomsForBoss, this.minRoomsForBoss + 1, false);
					...
					} else // all other build areas that aren't the largest
					{
						...
						// CQRMain.logger.info("Added central struct to NOT largest area: {}",
						// structArea);
						this.grid.selectBlockOfCellsForBuilding(structArea, this.floorsPerLayer);
						this.addSupportIfFirstLayer(layer, structArea);
...
//				CQRMain.logger.info("Buildable areas was empty (break here).");
	...
	private void addSideStructures(RoomGrid.Area2D structArea, RoomGrid.Area2D buildArea) {
		for (BlockFace side : EnumFacingConstant.HORIZONTALS) {
			...
			// While there is still room to build in this direction, 75% chance to keep
			// going
	...
	private void addSupportIfFirstLayer(int layer, int gridIndexX, int gridIndexZ, int roomsX, int roomsZ) {
		if (layer == 0) {
			// get NW corner of the area and move it NW 1 square because of the extra N and
			// W walls on the sides
	...
	private void placeTowers() {
		for (int floor = 0; floor < this.usedFloors; floor += this.floorsPerLayer) {
			Set<BlockFace> sidesToCheck = EnumSet.allOf(BlockFace.class);
			...
			CellLoop: for (RoomGridCell cell : candidateCells) {
				for (BlockFace side : sidesToCheck) {
					boolean canBuild;

					if (floor == 0) {
						canBuild = this.grid.cellIsOuterEdge(cell, side) && this.grid.canAttachTower(cell, side);
					} else {
						canBuild = this.grid.adjacentCellIsWalkableRoof(cell, side)
								&& this.grid.canAttachTower(cell, side);
					}
					...
					if (canBuild) {
						...
						if (maxHeight > MIN_TOWER_FLOORS) {
							...
							this.addTower(cell.getGridPosition().move(side), height, side.getOppositeFace());
	...
	private void addTower(RoomGridPosition position, int height, BlockFace alignment) {
		int x = position.getX();
		int z = position.getZ();
		int startFloor = position.getFloor();

		// CQRMain.logger.info("Placing tower at {},{} on floor {} facing {}, size =
		// {}", x, z, startFloor, alignment,
		// this.roomSize);
...
//				CQRMain.logger.info("Tried to place a tower @ null cell");
				...
				tower = new CastleRoomTowerSquare(this.roomSize, this.floorHeight, alignment, this.roomSize, tower,
						cell.getFloor(), this.random);
				cell.setRoom(tower);
			}
		}
		...
		if (tower != null && this.grid.withinGridBounds(startFloor + height, x, z)) {
			...
			if (DungeonGenUtils.percentageRandom(50, this.random)) {
				...
				this.castleRoofs.add(CastleRoofFactory.createRoof(this.dungeon.getRandomTowerRoofType(this.random),
						startPos, tower.getRoomLengthX() + 1, tower.getRoomLengthZ() + 1));
			}
		}
	}

	private void placeBridges() {
		List<RoomGridCell> populated = this.grid.getAllCellsWhere(
				c -> c.isPopulated() && c.getFloor() > 0 && !(c.getRoom() instanceof CastleRoomReplacedRoof));
		for (RoomGridCell cell : populated) {

			// Get all directions from the room that could potentially be the start of a
			// bridge
			List<BlockFace> possibleDirections = cell.getPotentialBridgeDirections();
			if (!possibleDirections.isEmpty()) {

				// Filter the directions from this cell that meet min and max length
				// requirements
				List<BlockFace> validDirections = new ArrayList<>();

				for (BlockFace direction : possibleDirections) {
					List<RoomGridCell> bridgeCells = this.grid.getBridgeCells(cell, direction);
					if (bridgeCells.size() >= this.dungeon.getMinBridgeLength()
							&& bridgeCells.size() <= this.dungeon.getMaxBridgeLength()) {
						validDirections.add(direction);
					}
				}

				if (!validDirections.isEmpty()
						&& DungeonGenUtils.percentageRandom(this.dungeon.getBridgeChance(), this.random)) {
					Collections.shuffle(validDirections, this.random);
					final BlockFace selectedDirection = validDirections.get(0);
					...
					for (RoomGridCell bridgeCell : bridgeCells) {
						if (!bridgeCell.isPopulated()) {
							CastleRoomBridgeTop bridgeRoom = new CastleRoomBridgeTop(this.roomSize, this.floorHeight,
									selectedDirection, bridgeCell.getFloor(), this.random);
							bridgeCell.setRoom(bridgeRoom);
						}
					}

					RoomGridCell endCell = this.grid.getAdjacentCell(bridgeCells.get(bridgeCells.size() - 1),
							selectedDirection);
					if (endCell != null && endCell.isPopulated()) {
						endCell.addDoorOnSideCentered(selectedDirection.getOppositeFace(), EnumCastleDoorType.RANDOM,
								this.random);
						if (endCell.getRoom() instanceof CastleRoomWalkableRoof) {
							endCell.removeWall(selectedDirection.getOppositeFace());
						}
					}
				}
			}
		...
		while (!unTyped.isEmpty()) {
			...
			for (int x = 0; x < sizeX; x++) {
				for (int z = 0; z < sizeZ; z++) {
					RoomGridCell buildCell = this.grid.getCellAt(rootCell.getFloor(), rootCell.getGridX() + x,
							rootCell.getGridZ() + z);
					CastleRoomBase roomToBuild = RoomFactoryCastle.CreateGenericRoom(type, this.roomSize,
							this.floorHeight, buildCell.getFloor(), this.random);
	...
	private void addBossRooms() {
		...
		if (bossArea != null && bossArea.dimensionsAreAtLeast(this.minRoomsForBoss, this.minRoomsForBoss + 1)) {
			...
			// If the boss room is an even number of rooms wide on the stair side, we need
			// to use the double boss stairs
			boolean dualStairs = (shortSideLen % 2 == 0);

			Map<RoomGridPosition, BlockFace> possibleStairs = new HashMap<>();

			// Define which direction is along the long side/short side for help with
			// alignment
			BlockFace alongLongSide = horizontal ? BlockFace.EAST : BlockFace.SOUTH;
			BlockFace alongShortSide = horizontal ? BlockFace.SOUTH : BlockFace.EAST;
			...
			possibleStairs.put(closePos.move(alongLongSide, longSideLen - 1), alongLongSide.getOppositeFace());
			...
			if (!possibleStairs.isEmpty()) {
				...
				RoomGridPosition bottomOfBossStairs = topOfBossStairs.move(BlockFace.DOWN);
				BlockFace stairDoorSide = possibleStairs.get(topOfBossStairs);

				if (dualStairs) {
					RoomGridCell cell = this.grid.getCellAt(bottomOfBossStairs);
					CastleRoomBossStairMain stairMain = new CastleRoomBossStairMain(this.roomSize, this.floorHeight,
							stairDoorSide, bottomOfBossStairs.getFloor(), this.random);
					cell.setRoom(stairMain);

					cell = this.grid.getCellAt(bottomOfBossStairs.move(alongShortSide));
					CastleRoomBossStairEmpty stairEmpty = new CastleRoomBossStairEmpty(this.roomSize, this.floorHeight,
							stairDoorSide, bottomOfBossStairs.getFloor(), this.random);
					cell.setRoom(stairEmpty);

					cell = this.grid.getCellAt(topOfBossStairs);
					CastleRoomBossLandingMain landingMain = new CastleRoomBossLandingMain(this.roomSize,
							this.floorHeight, stairDoorSide, topOfBossStairs.getFloor(), this.random);
					cell.setRoom(landingMain);

					cell = this.grid.getCellAt(topOfBossStairs.move(alongShortSide));
					CastleRoomBossLandingEmpty landingEmpty = new CastleRoomBossLandingEmpty(this.roomSize,
							this.floorHeight, stairDoorSide, topOfBossStairs.getFloor(), this.random);
					cell.setRoom(landingEmpty);
				} else {
					RoomGridCell cell = this.grid.getCellAt(bottomOfBossStairs);
					CastleRoomStaircaseDirected stair = new CastleRoomStaircaseDirected(this.roomSize, this.floorHeight,
							stairDoorSide, bottomOfBossStairs.getFloor(), this.random);
					cell.setRoom(stair);

					cell = this.grid.getCellAt(topOfBossStairs);
					CastleRoomLandingDirectedBoss landing = new CastleRoomLandingDirectedBoss(this.roomSize,
							this.floorHeight, stair, topOfBossStairs.getFloor(), this.random);
					cell.setRoom(landing);
				}

				// calculate the position of the "root" (northwest) boss room relative to the
				// boss area
				rootPos = bossArea.start;

				if (stairDoorSide == BlockFace.SOUTH) { // Bump it south if the north edge contains the stairs
					rootPos = rootPos.move(BlockFace.SOUTH);
				} else if (stairDoorSide == BlockFace.EAST) { // Bump it east if the west edge contains the stairs
					rootPos = rootPos.move(BlockFace.EAST);
				}

				// Constuct the root (NW) boss room and add it to the grid
				// Only the "root" room contains any build logic, the rest are blank rooms just
				// to mark off a position in the grid
				...
				for (int x = 0; x < this.minRoomsForBoss; x++) {
					for (int z = 0; z < this.minRoomsForBoss; z++) {
						...
						RoomGridPosition emptyRoomPos = rootPos.move(BlockFace.EAST, x).move(BlockFace.SOUTH, z);

						RoomGridCell roofCell = this.grid.getCellAt(emptyRoomPos);
						CastleRoomRoofBossEmpty emptyRoom = new CastleRoomRoofBossEmpty(this.roomSize, this.floorHeight,
								emptyRoomPos.getFloor(), this.random);
						roofCell.setRoom(emptyRoom);
						roofCell.setBossRoomCell();
					}
				}

				// It is likely the boss room does not take up every square of the grid cells it
				// occupies
				// so move the boss room area a few squares to align it with the stairs
				BlockFace snapToSide = stairDoorSide.getOppositeFace(); // Direction we are moving the room
				if (snapToSide == BlockFace.NORTH) {
				...
				} else if (snapToSide == BlockFace.WEST) {
				...
				} else if (snapToSide == BlockFace.SOUTH) {
					int distFromEdge = (bossArea.sizeX * this.roomSize) - rootRoom.getStaticSize();
					int x = (distFromEdge / 2) + 1;
...
//			CQRMain.logger.warn("Error adding boss rooms: boss area was never set during castle shaping.");
	...
	public boolean cellValidForDirectedStairs(RoomGridPosition position, BlockFace direction) {
		...
		if (stairCell != null && stairCell.isBuildable() && roomToStairs != null && roomToStairs.isBuildable()) {
			...
			List<BlockFace> outerSides = new ArrayList<>(Arrays.asList(EnumFacingConstant.HORIZONTALS));
			outerSides.remove(direction);

			for (BlockFace side : outerSides) {
			...
			// At this point we have checked all sides for blocked pathing so it should be
			// OK
			return true;
		}
	...
	private void linkCellsOnFloor(int floor) {
		List<RoomGridCell> floorCells = this.grid
				.getAllCellsWhere(c -> c.isPopulated() && c.getFloor() == floor && !c.getRoom().isWalkableRoof());
	...
	private void linkCellToAdjacentCells(RoomGridCell cell) {
		...
		for (BlockFace direction : EnumFacingConstant.HORIZONTALS) {
			RoomGridCell adjacent = this.grid.getAdjacentCell(cell, direction);
			if (adjacent != null && adjacent.isPopulated()
					&& cell.getRoom().getRoomType() == adjacent.getRoom().getRoomType()) {
	...
	private void placeOuterDoors() {
		...
		for (int floor = 0; floor < this.usedFloors; floor += this.floorsPerLayer) {
			Set<BlockFace> doorDirections = EnumSet.noneOf(BlockFace.class); // Sides of this floor that already have
																				// exits

			final int f = floor;
			List<RoomGridCell> floorRooms = this.grid.getAllCellsWhere(r -> r.getFloor() == f && r.isPopulated()
					&& !r.getRoom().isTower() && !r.getRoom().isWalkableRoof());
	...
	private void addHallways() {
		for (int floor = 0; floor < (this.grid.getBossArea().start.getFloor() - 1); floor++) {
			List<RoomGrid.Area2D> largestAreas = this.grid.getAllGridAreasWhere(floor, RoomGridCell::isValidHallwayRoom,
					2, 2);
			if (!largestAreas.isEmpty()) {
				RoomGrid.Area2D hallwayArea = largestAreas.get(0);
				boolean horizontal = hallwayArea.sizeX == hallwayArea.sizeZ ? this.random.nextBoolean()
						: hallwayArea.sizeX > hallwayArea.sizeZ;

				if (horizontal) {
					int zIndex = DungeonGenUtils.randomBetweenGaussian(hallwayArea.getStartZ(), hallwayArea.getEndZ(),
							this.random);
					...
					for (RoomGridCell hallwayCell : hallwayCells) {
						hallwayCell.setRoom(new CastleRoomHallway(this.roomSize, this.floorHeight,
								CastleRoomHallway.Alignment.HORIZONTAL, hallwayCell.getFloor(), this.random));
					}

				} else {
					int xIndex = DungeonGenUtils.randomBetweenGaussian(hallwayArea.getStartX(), hallwayArea.getEndX(),
							this.random);
					...
					for (RoomGridCell hallwayCell : hallwayCells) {
						hallwayCell.setRoom(new CastleRoomHallway(this.roomSize, this.floorHeight,
								CastleRoomHallway.Alignment.VERTICAL, hallwayCell.getFloor(), this.random));
					}

					if (floor == 0) {
						if (this.random.nextBoolean()) {
							hallwayCells.get(0).addOuterWall(BlockFace.NORTH);
							hallwayCells.get(0).addDoorOnSideCentered(BlockFace.NORTH, EnumCastleDoorType.GRAND_ENTRY,
									this.random);
							hallwayCells.get(0).setReachable();
						} else {
							hallwayCells.get(hallwayCells.size() - 1).addOuterWall(BlockFace.SOUTH);
							hallwayCells.get(hallwayCells.size() - 1).addDoorOnSideCentered(BlockFace.SOUTH,
									EnumCastleDoorType.GRAND_ENTRY, this.random);
	...
	private void addStairCases() {
		for (int floor = 0; floor < this.usedFloors; floor++) {
			...
			for (RoomGridCell cell : candidateCells) {
				RoomGridCell aboveCell = this.grid.getAdjacentCell(cell, BlockFace.UP);
				if (aboveCell != null && aboveCell.needsRoomType() && !aboveCell.isOnFloorWithLanding()) {
					CastleRoomStaircaseSpiral stairs = new CastleRoomStaircaseSpiral(this.roomSize, this.floorHeight,
							cell.getFloor(), this.random);
					cell.setRoom(stairs);

					CastleRoomLandingSpiral landing = new CastleRoomLandingSpiral(this.roomSize, this.floorHeight,
							stairs, aboveCell.getFloor(), this.random);
	...
	private boolean buildDirectedStairsIfPossible(RoomGridCell cell) {
		BlockFace side = this.getValidStairDoorSide(cell);
		if (side != BlockFace.DOWN) {
			RoomGridCell aboveCell = this.grid.getAdjacentCell(cell, BlockFace.UP);

			CastleRoomStaircaseDirected stairs = new CastleRoomStaircaseDirected(this.roomSize, this.floorHeight, side,
					cell.getFloor(), this.random);
			cell.setRoom(stairs);
			cell.addDoorOnSideCentered(side, EnumCastleDoorType.RANDOM, this.random);

			aboveCell.setRoom(new CastleRoomLandingDirected(this.roomSize, this.floorHeight, stairs,
					aboveCell.getFloor(), this.random));
			aboveCell.setReachable();
	...
	// Returns direction that the door can face on directed stairs, or DOWN if no
	// valid direction
	private BlockFace getValidStairDoorSide(RoomGridCell cell) {
		RoomGridCell aboveCell = this.grid.getAdjacentCell(cell, BlockFace.UP);
		if (aboveCell != null && !aboveCell.isPopulated()) {
			for (BlockFace side : EnumFacingConstant.HORIZONTALS) {
				RoomGridCell adjacent = this.grid.getAdjacentCell(cell, side);
				if (adjacent != null && adjacent.needsRoomType()
						&& !this.grid.cellBordersRoomType(cell, EnumRoomType.LANDING_DIRECTED)
						&& !this.grid.cellBordersRoomType(adjacent, EnumRoomType.LANDING_DIRECTED)
						&& this.grid.adjacentCellIsSelected(aboveCell, side)
	...
	private class PathNode {
		...
		private BlockFace parentDirection;
		private RoomGridCell cell;
		private double f;
		private double g;
		private double h;

		private PathNode(PathNode parent, BlockFace parentDirection, RoomGridCell cell, double g, double h) {
			this.parent = parent;
		...
		public BlockFace getParentDirection() {
	...
	private void pathBetweenRooms() {
		...
		for (int floor = 0; floor < this.maxFloors; floor++) {
			...
			List<RoomGridCell> reachable = this.grid
					.getAllCellsWhere(c -> c.getFloor() == f && c.isValidPathDestination());
						...
						for (PathNode node : destToSrcPath) {
							...
							if (cell != null) {
								if (node.getParent() != null) {
									cell.addDoorOnSideRandomOffset(node.getParentDirection(), EnumCastleDoorType.RANDOM,
											this.random);
								}
...
//						CQRMain.logger.info("Failed to find path from {} to {}", srcRoom.getGridPosition(),
//								destRoom.getGridPosition());
					}
				} else {
//					CQRMain.logger.info("{} had no pathable rooms!", srcRoom);

					// Add doors where we can to make sure the player can get to the room in some
					// way
					for (BlockFace side : EnumFacingConstant.HORIZONTALS) {
					...
					// Move it to the reachable list (but don't mark it as reachable so other cells
					// don't path here)
	...
	private List<PathNode> findPathBetweenRooms(RoomGridCell startCell, RoomGridCell endCell,
			@Nullable Set<RoomGridCell> invalidCells) {
		...
		open.add(new PathNode(null, BlockFace.DOWN, startCell, 0, startCell.distanceTo(endCell)));
		...
		while (!open.isEmpty()) {
			...
			// add each neighbor node to closed list if it connectable and not closed
			// already
			for (BlockFace direction : EnumFacingConstant.HORIZONTALS) {
				...
				if (currentNode.getCell().reachableFromSide(direction)) {
					...
					if (neighborCell != null && neighborCell.isPopulated()
							&& neighborCell.reachableFromSide(direction.getOppositeFace())
							&& !invalidCells.contains(neighborCell)) {
						PathNode neighborNode = new PathNode(currentNode, direction.getOppositeFace(), neighborCell,
								neighborG, neighborCell.distanceTo(endCell));

						// should really do this with .contains() but I don't feel like doing the
						// overrides
	...
	private RoomGridCell findNearestReachableRoom(RoomGridCell origin, Set<RoomGridCell> pathableRooms) {
		...
		if (!sorted.isEmpty()) {
			sorted.sort((RoomGridCell c1, RoomGridCell c2) -> Double.compare(
					this.grid.distanceBetweenCells2D(origin, c1), (this.grid.distanceBetweenCells2D(origin, c2))));
	...
	private void determineWalls() {
		/*
		 * List<RoomGridCell> cells = this.grid.getAllCellsWhere(c -> c.isPopulated() &&
		 * !c.isBossArea()); for (RoomGridCell cell : cells) { if (cell.getRoom()
		 * instanceof CastleRoomWalkableRoof) { this.determineWalkableRoofWalls(cell); }
		 * else { this.determineNormalRoomWalls(cell); } }
		 */
	...
	private void determineNormalRoomWalls(RoomGridCell cell) {
		// If we are at the edge cells, we force adding the walls. Otherwise we don't
		// force
		// it so rooms like hallways don't add them by mistake.
		boolean outerSouth = !this.grid.adjacentCellIsFullRoom(cell, BlockFace.SOUTH);

		if (outerSouth) {
			cell.addOuterWall(BlockFace.SOUTH);
		} else {
			if (!cell.isConnectedToCell(this.grid.getAdjacentCell(cell, BlockFace.SOUTH))) {
				cell.addInnerWall(BlockFace.SOUTH);
			}
		}

		boolean outerEast = !this.grid.adjacentCellIsFullRoom(cell, BlockFace.EAST);

		if (outerEast) {
			cell.addOuterWall(BlockFace.EAST);
		} else {
			if (!cell.isConnectedToCell(this.grid.getAdjacentCell(cell, BlockFace.EAST))) {
				cell.addInnerWall(BlockFace.EAST);
			}
		}

		if (!this.grid.adjacentCellIsFullRoom(cell, BlockFace.NORTH)) {
			cell.addOuterWall(BlockFace.NORTH);
		}

		if (!this.grid.adjacentCellIsFullRoom(cell, BlockFace.WEST)) {
			cell.addOuterWall(BlockFace.WEST);
		}
	}
		...
		for (RoomGrid.Area2D roofArea : roofAreas) {
			if (this.random.nextBoolean()) {
				...
				for (RoomGridPosition areaPos : roofArea.getPositionList()) {
					...
					this.grid.getCellAt(areaPos).setRoom(new CastleRoomReplacedRoof(this.roomSize, this.floorHeight,
							areaPos.getFloor(), this.random));
				}
			}
		}
	...
	private void addRoofFromRoofArea(RoomGrid.Area2D roofArea) {
		...
		this.castleRoofs.add(
				CastleRoofFactory.createRoof(this.dungeon.getRandomRoofType(this.random), roofStart, sizeX, sizeZ));
	}
