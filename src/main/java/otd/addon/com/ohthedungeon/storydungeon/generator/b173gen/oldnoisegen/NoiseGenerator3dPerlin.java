package otd.addon.com.ohthedungeon.storydungeon.generator.b173gen.oldnoisegen;
...
public class NoiseGenerator3dPerlin {
	...
	public double generateNoise(double xPos, double yPos, double zPos) {
		...
		return lerp(fz,
				lerp(fy, lerp(fx, grad3d(permutations[a2], x, y, z), grad3d(permutations[a5], x - 1.0D, y, z)),
						lerp(fx, grad3d(permutations[a3], x, y - 1.0D, z),
								grad3d(permutations[a6], x - 1.0D, y - 1.0D, z))),
				lerp(fy, lerp(fx, grad3d(permutations[a2 + 1], x, y, z - 1.0D),
	...
	public void generateNoiseArray(double array[], double xPos, double yPos, double zPos, int xSize, int ySize,
			int zSize, double gridX, double gridY, double gridZ, double a) {
		if (ySize == 1) {
			int index = 0;
			...
			for (int dx = 0; dx < xSize; dx++) {
				...
				for (int dz = 0; dz < zSize; dz++) {
					...
					double d9 = lerp(d17, grad2d(permutations[j1], x, z), grad3d(permutations[l1], x - 1.0D, 0.0D, z));
					double d11 = lerp(d17, grad3d(permutations[j1 + 1], x, 0.0D, z - 1.0D),
							grad3d(permutations[l1 + 1], x - 1.0D, 0.0D, z - 1.0D));
		...
		for (int dx = 0; dx < xSize; dx++) {
			...
			for (int dz = 0; dz < zSize; dz++) {
				...
				for (int dy = 0; dy < ySize; dy++) {
					...
					int k7 = j7 & 0xff;
					y -= j7;
					double d27 = y * y * y * (y * (y * 6D - 15D) + 10D);
					if (dy == 0 || k7 != i2) {
						i2 = k7;
						...
						d13 = lerp(d22, grad3d(permutations[k2], x, y, z), grad3d(permutations[k3], x - 1.0D, y, z));
						d15 = lerp(d22, grad3d(permutations[l2], x, y - 1.0D, z),
								grad3d(permutations[l3], x - 1.0D, y - 1.0D, z));
						d16 = lerp(d22, grad3d(permutations[k2 + 1], x, y, z - 1.0D),
								grad3d(permutations[k3 + 1], x - 1.0D, y, z - 1.0D));
						d18 = lerp(d22, grad3d(permutations[l2 + 1], x, y - 1.0D, z - 1.0D),
								grad3d(permutations[l3 + 1], x - 1.0D, y - 1.0D, z - 1.0D));
					}
