...
public abstract class Database {
	...
	public Integer getDungeonCount(String string) {
		...
		try {
			...
			int res = 0;
			while (rs.next()) {
				if (rs.getString("player").equalsIgnoreCase(string.toLowerCase())) { // Tell database to search for the
																						// player you sent into the
																						// method. e.g getTokens(sam) It
																						// will look for sam.
					res++;
				}
			}
...
// Now we need methods to save things to the database
	private void addDungeon(String player, Integer dx, Integer dy) {
		...
		try {
			conn = getSQLConnection();
			ps = conn.prepareStatement("REPLACE INTO " + table + " (key,player,dx,dy) VALUES(?,?,?,?)"); // IMPORTANT.
																											// In SQLite
																											// class, We
																											// made 3
																											// colums.
