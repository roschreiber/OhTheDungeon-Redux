...
public class NoiseGeneratorOctaves3D {
	...
	public NoiseGeneratorOctaves3D(Random random, int i) {
		...
		for (int j = 0; j < i; j++) {
			generatorCollection[j] = new NoiseGenerator3dPerlin(random, true);
		}
	}
	...
	public double[] generateNoiseArray(double ad[], double x, double y, double z, int xSize, int ySize, int zSize,
			double gridX, double gridY, double gridZ) {
		if (ad == null) {
		...
		for (int i1 = 0; i1 < octaves; i1++) {
			generatorCollection[i1].generateNoiseArray(ad, x, y, z, xSize, ySize, zSize, gridX * frequency,
					gridY * frequency, gridZ * frequency, frequency);
			frequency /= 2D;
		}
	...
	public double[] generateNoiseArray(double ad[], int x, int z, int xSize, int zSize, double gridX, double gridZ,
			double d2) {
