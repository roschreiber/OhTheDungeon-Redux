...
public class FMB_RMF {
	...
	private static double noise(double x, double y, double z) {
		...
		return lerp(w,
				lerp(v, lerp(u, grad(ms_p[AA], x, y, z), grad(ms_p[BA], x - 1, y, z)),
						lerp(u, grad(ms_p[AB], x, y - 1, z), grad(ms_p[BB], x - 1, y - 1, z))),
				lerp(v, lerp(u, grad(ms_p[AA + 1], x, y, z - 1), grad(ms_p[BA + 1], x - 1, y, z - 1)),
						lerp(u, grad(ms_p[AB + 1], x, y - 1, z - 1), grad(ms_p[BB + 1], x - 1, y - 1, z - 1))));
	}
	...
	public double noise_RidgedMultiFractal(final double x, final double y, final double z, final int octaves,
			final float lacunarity, final float gain, final float offset) {
		double sum = 0;
	...
	public double noise_FractionalBrownianMotion(final double x, final double y, final double z, final int octaves,
