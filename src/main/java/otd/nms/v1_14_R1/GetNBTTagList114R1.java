...
public class GetNBTTagList114R1 implements GetNBTTagList {
	public Object get(Random rand, int level, SpawnPotential sp) {
		...
		if (sp.name.equals(Spawner.getName(Spawner.ZOMBIE))) {
			for (int i = 0; i < 24; ++i) {
				...
				mob = (net.minecraft.server.v1_14_R1.NBTTagCompound) sp.equipHands(mob,
				...
				potentials.add((net.minecraft.server.v1_14_R1.NBTBase) sp.getPotential(mob));
			}
		...
		potentials.add((net.minecraft.server.v1_14_R1.NBTBase) sp
				.getPotential(sp.getRoguelike(level, sp.name, new net.minecraft.server.v1_14_R1.NBTTagCompound())));
		return potentials;
	}
}
