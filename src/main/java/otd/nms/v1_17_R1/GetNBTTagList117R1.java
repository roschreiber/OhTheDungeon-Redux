...
public class GetNBTTagList117R1 implements GetNBTTagList {
	public Object get(Random rand, int level, SpawnPotential sp) {
		...
		if (sp.name.equals(Spawner.getName(Spawner.ZOMBIE))) {
			for (int i = 0; i < 24; ++i) {
				...
				potentials.add((net.minecraft.nbt.NBTBase) sp.getPotential(mob));
		...
		potentials.add((net.minecraft.nbt.NBTBase) sp
