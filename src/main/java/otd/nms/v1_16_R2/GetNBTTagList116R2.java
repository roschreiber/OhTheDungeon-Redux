...
public class GetNBTTagList116R2 implements GetNBTTagList {
	public Object get(Random rand, int level, SpawnPotential sp) {
		...
		if (sp.name.equals(Spawner.getName(Spawner.ZOMBIE))) {
			for (int i = 0; i < 24; ++i) {
				...
				mob = (net.minecraft.server.v1_16_R2.NBTTagCompound) sp.equipHands(mob,
				...
				potentials.add((net.minecraft.server.v1_16_R2.NBTBase) sp.getPotential(mob));
			}
		...
		potentials.add((net.minecraft.server.v1_16_R2.NBTBase) sp
				.getPotential(sp.getRoguelike(level, sp.name, new net.minecraft.server.v1_16_R2.NBTTagCompound())));
		return potentials;
	}
}
