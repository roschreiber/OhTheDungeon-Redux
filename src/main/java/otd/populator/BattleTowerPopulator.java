...
public class BattleTowerPopulator extends IPopulator {
	...
	public boolean generateDungeon(World world, Random random, Chunk chunk) {
		...
		if (random.nextInt(100) == 0)
			...
			Biome b = MultiVersion.getBiome(world, rx, rz);
			Set<Integer> typelist = new HashSet<>();
			Set<Type> set = BiomeDictionary.getTypes(b);
			if (set.contains(Type.SANDY))
				typelist.add(3);
			if (set.contains(Type.COLD))
				typelist.add(4);
			if (set.contains(Type.SNOWY))
				typelist.add(4);
			if (set.contains(Type.MOUNTAIN))
				typelist.add(5);
			if (set.contains(Type.HILLS))
				typelist.add(5);
			if (set.contains(Type.WET))
				typelist.add(2);
			if (set.contains(Type.OCEAN))
				typelist.add(2);
			if (set.contains(Type.RIVER))
				typelist.add(2);
			if (set.contains(Type.JUNGLE))
				typelist.add(7);
			if (set.contains(Type.PLAINS))
				typelist.add(1);
		...
		if (WorldConfig.wc.dict.containsKey(world.getName())) {
			bottom = WorldConfig.wc.dict.get(world.getName()).worldParameter.bottom;
		}
		if (ry > bottom + 5)
			under = random.nextBoolean();
		...
		Bukkit.getScheduler().runTaskLater(Main.instance, () -> {
			DungeonGeneratedEvent event = new DungeonGeneratedEvent(world, chunks0, DungeonType.BattleTower, rx, ry,
					rz);
