...
public class DungeonPopulator extends BlockPopulator {
	...
	public void populate(final World world, final Random random, final Chunk source) {
		...
		if (swc.castle.doNaturalSpawn) {
			generator.add(swc.castle_weight, new CastlePopulator());
		}
		...
		if (distance < 25)
			distance *= choose.getDungeonConstant();

		Random mrand = new Random(world.getSeed() + world_name.hashCode() + (2027 * (long) (source.getX() / distance))
		...
		if (biomes.contains(MultiVersion.getBiome(world, rx, rz).toString()))
			return;
	...
	public static boolean isDungeonEnabled(SimpleWorldConfig swc) {
		return swc.roguelike.doNaturalSpawn || swc.doomlike.doNaturalSpawn || swc.battletower.doNaturalSpawn
				|| swc.smoofydungeon.doNaturalSpawn || swc.draylar_battletower.doNaturalSpawn
				|| swc.ant_man_dungeon.doNaturalSpawn || swc.aether_dungeon.doNaturalSpawn
				|| swc.lich_tower.doNaturalSpawn || swc.castle.doNaturalSpawn
