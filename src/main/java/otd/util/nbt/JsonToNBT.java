...
/**
 * An almost direct copy of net.minecraft.nbt.JsonToNBT, but strips quotes from
 * field names.
 *
 */
public class JsonToNBT {
	...
	static JsonToNBT.Any nameValueToNBT(String key, String value) throws NBTException {
		...
		if (key.startsWith("\"") && key.endsWith("\""))
			key = key.substring(1, key.length() - 1); // this is the only bit I changed
		...
		if (value.startsWith("{")) {
			...
			for (jsontonbt$compound = new JsonToNBT.Compound(key); value.length() > 0; value = value
			...
			for (jsontonbt$list = new JsonToNBT.List(key); value.length() > 0; value = value
	...
	public abstract static class Any {
		 ...
		 * 
		 * @return an {@link NBTBase} which can be safely cast to the type represented
		 *         by this class.
		 */
	...
	public static class Compound extends JsonToNBT.Any {
		...
		public java.util.List<JsonToNBT.Any> tagList = Lists.<JsonToNBT.Any>newArrayList();
		...
		public Object parse() throws NBTException {
			return MultiVersion.compoundParse == null ? null : MultiVersion.compoundParse.parse(this);
		}
	}
	...
	public static class List extends JsonToNBT.Any {
		...
		@Override
		public Object parse() throws NBTException {
			return MultiVersion.listParse == null ? null : MultiVersion.listParse.parse(this);
		}
	}
		...
		public Object parse() throws NBTException {
			try {
				if (DOUBLE.matcher(this.jsonValue).matches()) {
					return MultiVersion.primitiveParse == null ? null : MultiVersion.primitiveParse.parseDouble(this);
				}

				if (FLOAT.matcher(this.jsonValue).matches()) {
					return MultiVersion.primitiveParse == null ? null : MultiVersion.primitiveParse.parseFloat(this);
				}

				if (BYTE.matcher(this.jsonValue).matches()) {
					return MultiVersion.primitiveParse == null ? null : MultiVersion.primitiveParse.parseByte(this);
				}

				if (LONG.matcher(this.jsonValue).matches()) {
					return MultiVersion.primitiveParse == null ? null : MultiVersion.primitiveParse.parseLong(this);
				}

				if (SHORT.matcher(this.jsonValue).matches()) {
					return MultiVersion.primitiveParse == null ? null : MultiVersion.primitiveParse.parseShort(this);
				}

				if (INTEGER.matcher(this.jsonValue).matches()) {
					return MultiVersion.primitiveParse == null ? null : MultiVersion.primitiveParse.parseInteger(this);
				}

				if (DOUBLE_UNTYPED.matcher(this.jsonValue).matches()) {
					return MultiVersion.primitiveParse == null ? null
							: MultiVersion.primitiveParse.parseDoubleUntyped(this);
				}

				if ("true".equalsIgnoreCase(this.jsonValue) || "false".equalsIgnoreCase(this.jsonValue)) {
					return MultiVersion.primitiveParse == null ? null : MultiVersion.primitiveParse.parseBoolean(this);
				}
			} catch (NumberFormatException var6) {
				this.jsonValue = this.jsonValue.replaceAll("\\\\\"", "\"");
				return MultiVersion.primitiveParse == null ? null
						: MultiVersion.primitiveParse.parseString(this.jsonValue);
			}
			...
			if (this.jsonValue.startsWith("[") && this.jsonValue.endsWith("]")) {
				...
				try {
					...
					return MultiVersion.primitiveParse == null ? null
							: MultiVersion.primitiveParse.parseIntArray(this, aint);
				...
				return MultiVersion.primitiveParse == null ? null
						: MultiVersion.primitiveParse.parseString(stringbuilder.toString());
			}
		}
	}
}