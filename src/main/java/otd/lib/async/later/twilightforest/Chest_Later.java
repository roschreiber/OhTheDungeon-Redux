...
public class Chest_Later extends Later {
	...
	public static boolean generate_later(AsyncWorldEditor world, Coord coord, Random random, TFTreasure treasureType,
			Material chest) {
	...
	public void doSomethingInChunk(Chunk c) {
		int bx = coord.getX() % 16;
		int by = coord.getY() + this.y;
		int bz = coord.getZ() % 16;
		if (bx < 0)
			bx = bx + 16;
		if (bz < 0)
			bz = bz + 16;
		...
		if (bs instanceof Chest) {
			...
			if (WorldConfig.wc.dict.containsKey(world_name)
					&& !WorldConfig.wc.dict.get(world_name).lich_tower.builtinLoot) {
				builtin = false;
			}
