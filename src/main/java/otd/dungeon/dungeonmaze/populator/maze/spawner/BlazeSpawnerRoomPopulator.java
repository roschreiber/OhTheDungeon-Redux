...
public class BlazeSpawnerRoomPopulator extends MazeRoomBlockPopulator {
	...
	private static final BlockData NETHER_BRICK_STAIRS0 = Bukkit
			.createBlockData("minecraft:nether_brick_stairs[half=bottom,shape=straight,facing=east]");
	private static final BlockData NETHER_BRICK_STAIRS1 = Bukkit
			.createBlockData("minecraft:nether_brick_stairs[half=bottom,shape=straight,facing=west]");
	private static final BlockData NETHER_BRICK_STAIRS2 = Bukkit
			.createBlockData("minecraft:nether_brick_stairs[half=bottom,shape=straight,facing=south]");
	private static final BlockData NETHER_BRICK_STAIRS3 = Bukkit
			.createBlockData("minecraft:nether_brick_stairs[half=bottom,shape=straight,facing=north]");
...
//		final int y = args.getChunkY();
		...
		for (int xx = x; xx <= x + 7; xx += 1)
			for (int zz = z; zz <= z + 7; zz += 1)
				world.setChunkType(xx, yFloor, zz, Material.NETHER_BRICKS);

		// Cobblestone layer underneath the stone floor
		for (int xx = x; xx <= x + 7; xx += 1)
			for (int zz = z + 1; zz <= z + 6; zz += 1)
				world.setChunkType(xx, yFloor - 1, zz, Material.COBBLESTONE);
		...
		// Generate corners
		for (int yy = yFloor + 1; yy < yCeiling; yy++) {
			world.setChunkType(x, yy, z, Material.NETHER_BRICKS);
			world.setChunkType(x + 7, yy, z, Material.NETHER_BRICKS);
			world.setChunkType(x, yy, z + 7, Material.NETHER_BRICKS);
			world.setChunkType(x + 7, yy, z + 7, Material.NETHER_BRICKS);
		}

		// Generate fences in the corners
		for (int yy = yFloor + 1; yy < yCeiling; yy++) {
			world.setChunkType(x + 1, yy, z, Material.NETHER_BRICK_FENCE);
			world.setChunkType(x, yy, z + 1, Material.NETHER_BRICK_FENCE);
			world.setChunkType(x + 6, yy, z, Material.NETHER_BRICK_FENCE);
			world.setChunkType(x + 7, yy, z + 1, Material.NETHER_BRICK_FENCE);
			world.setChunkType(x + 1, yy, z + 7, Material.NETHER_BRICK_FENCE);
			world.setChunkType(x, yy, z + 6, Material.NETHER_BRICK_FENCE);
			world.setChunkType(x + 6, yy, z + 7, Material.NETHER_BRICK_FENCE);
			world.setChunkType(x + 7, yy, z + 6, Material.NETHER_BRICK_FENCE);
		}

		// Generate platform in the middle
		for (int xx = x + 2; xx <= x + 5; xx += 1)
			for (int zz = z + 2; zz <= z + 5; zz += 1)
				world.setChunkType(xx, yFloor + 1, zz, Material.NETHER_BRICKS);

		// Generate stairs off the platform
		world.setChunkData(x + 3, yFloor + 1, z + 2, NETHER_BRICK_STAIRS2);
		world.setChunkData(x + 4, yFloor + 1, z + 2, NETHER_BRICK_STAIRS2);

		world.setChunkData(x + 3, yFloor + 1, z + 5, NETHER_BRICK_STAIRS3);
		world.setChunkData(x + 4, yFloor + 1, z + 5, NETHER_BRICK_STAIRS3);

		world.setChunkData(x + 2, yFloor + 1, z + 3, NETHER_BRICK_STAIRS0);
		world.setChunkData(x + 2, yFloor + 1, z + 4, NETHER_BRICK_STAIRS0);

		world.setChunkData(x + 5, yFloor + 1, z + 3, NETHER_BRICK_STAIRS1);
		world.setChunkData(x + 5, yFloor + 1, z + 4, NETHER_BRICK_STAIRS1);

		// Generate poles on the platform
		world.setChunkType(x + 2, yFloor + 2, z + 2, Material.NETHER_BRICK_FENCE);
		world.setChunkType(x + 5, yFloor + 2, z + 2, Material.NETHER_BRICK_FENCE);
		world.setChunkType(x + 2, yFloor + 2, z + 5, Material.NETHER_BRICK_FENCE);
		world.setChunkType(x + 5, yFloor + 2, z + 5, Material.NETHER_BRICK_FENCE);

		// Generate the spawner
		if (SpawnerUtils.isBlazeAllowed(world.getWorld())) {
			int spawnerX = x + 3 + rand.nextInt(2);
			int spawnerY = yFloor + 2;
			int spawnerZ = z + 3 + rand.nextInt(2);

			Spawner_Later later = new Spawner_Later(chunkx * 16 + spawnerX, spawnerY, chunkz * 16 + spawnerZ,
					EntityType.BLAZE);
		...
		world.setChunkType(x + 3, yFloor, z + 3, Material.CHEST);
		{
			Chest_Later later = new Chest_Later(chunkx * 16 + x + 3, yFloor, chunkz * 16 + z + 3, rand, contents);
			world.addLater(later);
		}
...
//		ChestUtils.addItemsToChest(chest1, contents, true, rand, world);
		...
		world.setChunkType(x + 4, yFloor, z + 4, Material.CHEST);
		{
			Chest_Later later = new Chest_Later(chunkx * 16 + x + 4, yFloor, chunkz * 16 + z + 4, rand, contents2);
			world.addLater(later);
