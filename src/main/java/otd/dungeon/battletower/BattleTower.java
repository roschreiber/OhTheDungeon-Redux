				...
				for (int xIterator = -7; xIterator < 7; xIterator++) // do each X
				{
					for (int zIterator = -7; zIterator < 7; zIterator++) // do each Z
					{
						...
						if (zIterator == -7) // last row, 14
						{
							...
							{
								buildWallPiece(world, iCurrent, jCurrent, zCurrent, towerWallBlockMaterial, floor,
										floorIterator);
							}
							continue;
						}
							...
							if (zIterator == -6) // row 13 extra
							{
								if (xIterator == (floorIterator + 1) % 7 - 3) // stairwell!!
								{
									if (!(underground && floor == 1)) {
										Directional dir = (Directional) Bukkit
												.createBlockData(towerChosen.getStairBlockMaterial());
										dir.setFacing(BlockFace.EAST);
										world.getBlockAt(iCurrent, jCurrent, zCurrent).setBlockData(dir, false);
									}
									if (floorIterator == 5) {
										world.getBlockAt(iCurrent - 7, jCurrent, zCurrent)
												.setBlockData(towerFloorBlockData, false);
									}
									if (floorIterator == 6 && topFloor) // top ledge part
									{
										buildWallPiece(world, iCurrent, jCurrent, zCurrent, towerWallBlockMaterial,
												floor, floorIterator);
									}
									continue;
								}
						...
						if (zIterator == -4 || zIterator == -3 || zIterator == 2 || zIterator == 3) // rows 11, 10, 5, 4
						{
							...
							if (world.getBlockAt(iCurrent, jCurrent, zCurrent).getType() != Material.CHEST) // tower
																											// inside
																											// space
							{
								world.getBlockAt(iCurrent, jCurrent, zCurrent).setType(Material.AIR, false);
...
//					world.spawnEntity(new Location(world, ix + 0.5D, builderHeight + 6, kz + 0.5D),
...
//				}
			...
			if (towerChosen != TowerTypes.Null) {
				...
				if (!best_chest) {
					...
					if (box)
						material1 = Material.SHULKER_BOX;
					else
						material1 = Material.CHEST;
	...
	private static void buildWallPiece(World world, int i, int j, int k, Material towerWallBlockID, int floor,
			int floorIterator) {
